...
%DIF DEL ../V3/Pfeil-Hydra-Manuscript-V3.tex   Thu Jan  9 23:12:55 2020
%DIF ADD Pfeil-Hydra-Manuscript-V4.tex         Mon Mar 16 01:04:45 2020
...
\\
\bigskip
\textbf{1} Department of Biomolecular Engineering, University of California, Santa Cruz, \DIFaddbegin \DIFadd{Santa Cruz, }\DIFaddend California, United States of America
\\
\textbf{2} Genomics Institute, University of California, Santa Cruz, \DIFaddbegin \DIFadd{Santa Cruz, }\DIFaddend California, United States of America
\\
\textbf{3} Department of Molecular, Cell and Developmental Biology, University of California, Santa Cruz, \DIFaddbegin \DIFadd{Santa Cruz, }\DIFaddend California, United States of America
\\
...
\\
\textbf{6} Howard Hughes Medical Institute, University of California, Santa Cruz, \DIFaddbegin \DIFadd{Santa Cruz, }\DIFaddend California, United States of America
\\
\bigskip
...
%DIFAUXCMD
\DIFdel{Traditional parametric models, like the finite mixture model, use a fixed number of parameters (i.e. number of clusters). Over- or underfitting can occur when the parametric model does not reflect the underlying data \mbox{%DIFAUXCMD
\cite{teh2010dirichlet}}\hspace{0pt}%DIFAUXCMD
. Unlike the finite mixture model, the Dirichlet process mixture model (DPMM) represents a theoretically infinite number of clusters and can adapt the number of clusters based on prior belief and the data \mbox{%DIFAUXCMD
\cite{gelmanBayesianDataAnalysis2013, antoniakMixturesDirichletProcesses1974, teh2010dirichlet}}\hspace{0pt}%DIFAUXCMD
.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The Dirichlet process ($DP$) is an infinite dimensional extension of the Dirichlet distribution \mbox{%DIFAUXCMD
\cite{fergusonBayesianAnalysisNonparametric1973} }\hspace{0pt}%DIFAUXCMD
and is commonly used as a prior distribution for infinite mixture models \mbox{%DIFAUXCMD
\cite{muller2004nonparametric, gorurDirichletProcessGaussian2010}}\hspace{0pt}%DIFAUXCMD
...
%DIFDELCMD < %%%
\DIFdel{We briefly describe the stick-breaking construction of the Dirichlet process $G \sim DP(\alpha, H)$. Consider a stick of unit length. To generate an infinite number of mixing weights $\pi_1, \pi_2, ..., \pi_k$ for the DPMM, first break a stick of unit length at $\nu \in [0, 1]$ where $\nu$ is sampled from a Beta distribution, and set $\pi_1$ to be the length of the first piece. We repeat this process using the remainder of the stick for each $\pi_k$. The DP is truncated to the number of clusters $K$, which was shown to accurately approximate the infinite posterior for large K \mbox{%DIFAUXCMD
...
%DIFDELCMD < %%%
\DIFdel{This construction generates the marginal of the Dirichlet process, which follows a Dirichlet distribution. Samples from the marginal distribution are finite, discrete, and sum to $1$ \mbox{%DIFAUXCMD
\cite{fergusonBayesianAnalysisNonparametric1973}}\hspace{0pt}%DIFAUXCMD
...
%DIFDELCMD < %%%
\DIFdel{To improve our methods ability to scale to larger datasets, we incorporated the bnpy memoized online variational inference algorithm (moVB) \mbox{%DIFAUXCMD
\cite{hughes2013memoized} }\hspace{0pt}%DIFAUXCMD
into our analysis framework. The moVB algorithm uses variational inference to approximate the posterior distribution and interleaves birth, merge, and delete moves to avoid local optima and remove redundant clusters \mbox{%DIFAUXCMD
\cite{hughesBnpyReliableScalable}}\hspace{0pt}%DIFAUXCMD
. We found that the moVB algorithm accurately identified the number of clustering on validation datasets (}%DIFDELCMD < \nameref{S1_Fig}%%%
...
%DIFAUXCMD
\DIFdel{We developed a Bayesian non-parametric clustering framework for identifying biological and technical variation in large cancer gene expression datasets without the need for a reference normal dataset. To our knowledge, this is the first reproducible and widely deployable implementation of a non-parametric mixture model framework designed to overcome the challenges of precision oncology gene expression analysis. The hydra pipeline is an open source software tool hosted on GitHub (}%DIFDELCMD < \url{www.github.com/jpfeil/hydra}%%%
\DIFdel{). A Docker container is available for deployment across environments (}%DIFDELCMD < \url{https://hub.docker.com/r/jpfeil/hydra}%%%
\DIFdel{).
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The hydra framework contains three main command-line tools: }\textit{\DIFdel{filter}}%DIFAUXCMD
\DIFdel{, }\textit{\DIFdel{enrich}}%DIFAUXCMD
\DIFdel{, and }\textit{\DIFdel{sweep}} %DIFAUXCMD
\DIFdel{(Fig \ref{overview}). The }\textit{\DIFdel{filter}} %DIFAUXCMD
\DIFdel{command is run first to isolate the multimodally expressed genes using a univariate Dirichlet Process Gaussian Mixture Model (DP-GMM). There are two methods for analyzing the resulting set of multimodally expressed genes. The }\textit{\DIFdel{enrich}} %DIFAUXCMD
\DIFdel{method, which subsets to the genes found to be significantly enriched in biological pathways, and the }\textit{\DIFdel{sweep}} %DIFAUXCMD
...
{%DIFAUXCMD
%DIFDELCMD < {\bf %%%
\DIFdelFL{Overview of the hydra framework tools.}%DIFDELCMD < }
%DIFDELCMD < 		%%%
\DIFdelFL{A: Suggested workflow for applying hydra framework tools to identify clinically relevant gene expression subtypes. B: The hydra }\textit{\DIFdelFL{filter}} %DIFAUXCMD
\DIFdelFL{command removes unimodally distributed genes which greatly reduces the number of genes in downstream clustering analysis. C: The hydra }\textit{\DIFdelFL{enrich}} %DIFAUXCMD
\DIFdelFL{command takes the multimodally expressed genes and returns enriched gene sets. The enriched gene set genes are used for multivariate clustering of samples. D: The hydra
	    }\textit{\DIFdelFL{sweep}} %DIFAUXCMD
...
%DIFDELCMD < %%%
\DIFdel{The }\textit{\DIFdel{filter}} %DIFAUXCMD
...
%DIFDELCMD < %%%
\DIFdel{The }\textit{\DIFdel{enrich}} %DIFAUXCMD
\DIFdel{(Fig \ref{overview}C) and }\textit{\DIFdel{sweep}} %DIFAUXCMD
\DIFdel{(Fig \ref{overview}D) routines are two independent analyses to explore multimodal expression in cancer gene expression cohorts. In addition to identifying expression variation within a disease context, we also found that multimodally expressed genes that participate in a biological pathway tend to have correlated expression distributions. This insight facilitates the detection of multimodal expression signatures by enriching for genes that have multimodal expression distributions and participate in known biological processes. The hydra software comes prepackaged with popular gene sets, including the Molecular Signatures Database (MSigDB) \mbox{%DIFAUXCMD
\cite{liberzonMolecularSignaturesDatabase2011}}\hspace{0pt}%DIFAUXCMD
, the Gene Ontology terms \mbox{%DIFAUXCMD
\cite{Ashburner2000, gene2018gene}}\hspace{0pt}%DIFAUXCMD
, and the EnrichmentMap gene sets \mbox{%DIFAUXCMD
\cite{merico2010enrichment}}\hspace{0pt}%DIFAUXCMD
. The gene set database is configurable, so additional gene sets can be added at runtime.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The }\textit{\DIFdel{enrich}} %DIFAUXCMD
\DIFdel{command uses a hypergeometric test \mbox{%DIFAUXCMD
\cite{yuClusterProfilerPackageComparing2012} }\hspace{0pt}%DIFAUXCMD
to discover enrichment of multimodally expressed genes within a user-defined database of gene sets. This creates a list of gene sets and a list of enriched gene set genes. The }\textit{\DIFdel{enrich}} %DIFAUXCMD
...
%DIFDELCMD < %%%
\DIFdel{The implementation of the }\textit{\DIFdel{enrich}} %DIFAUXCMD
\DIFdel{method includes an important parameter known as the minimum component probability. The minimum component probability is the probability of placing a sample within the smallest expression cluster. This is an additional filter to remove multimodally expressed genes that influence a relatively small subset of tumor samples. This parameter gives the user the ability to subset the enriched genes to those that influence a greater number of patients. To aid in the exploration of minimum component thresholds, we implemented a }\textit{\DIFdel{scan}} %DIFAUXCMD
\DIFdel{sub-routine. The }\textit{\DIFdel{scan}} %DIFAUXCMD
\DIFdel{routine tunes the analysis with respect to the constraints of the available data (e.g. number of samples and number of genes), which is an important factor in pediatric cancer research since data is often difficult to obtain and so datasets are relatively small. We recommend setting this threshold such that the number of genes is less than the number of samples because otherwise the inference may become unstable \mbox{%DIFAUXCMD
\cite{caiConstrainedL1Minimization2011}}\hspace{0pt}%DIFAUXCMD
.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The }\textit{\DIFdel{sweep}} %DIFAUXCMD
\DIFdel{routine identifies differentially expressed gene sets and can be used as an alternative to single-sample GSEA \mbox{%DIFAUXCMD
\cite{subramanianGeneSetEnrichment2005}}\hspace{0pt}%DIFAUXCMD
. For each gene set, a multivariate DP-GMM is applied to determine if more than one expression cluster is present within the gene set. This approach is useful when curated gene sets are available for the disease of interest, but manual inspection of each gene set is not feasible. Reducing the genes to multimodally expressed genes facilitates the detection of differentially expressed gene sets. Existing gene set enrichment tools are known to under-perform when the expression is correlated \mbox{%DIFAUXCMD
\cite{tamayoLimitationsSimpleGene2016}}\hspace{0pt}%DIFAUXCMD
, but our approach is designed to identify distinct correlation structures within gene expression datasets.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{We have also implemented routines for cluster profiling and N-of-1 tumor analysis. These routines are accessible within the docker container using the Jupyter notebook command. Cluster profiling analysis of clusters derived from the }\textit{\DIFdel{enrich}} %DIFAUXCMD
\DIFdel{or }\textit{\DIFdel{sweep}} %DIFAUXCMD
\DIFdel{routines includes GSEA \mbox{%DIFAUXCMD
...
%DIFDELCMD < %%%
\DIFdel{The N-of-1 tumor analysis routine classifies a new gene expression profile into one of the inferred clusters, calculates a gene-level z-score for that sample relative to the normalized expression distribution, and performs standard GSEA using a preranked list of z-score values \mbox{%DIFAUXCMD
\cite{korotkevichFastGeneSet2019}}\hspace{0pt}%DIFAUXCMD
...
%DIFAUXCMD
%DIF <  For figure citations, please use "Fig" instead of "Figure".
\DIFdel{We first tested the hydra framework’s ability to detect differential pathway expression using synthetic cancer data. We compared hydra }\textit{\DIFdel{sweep}} %DIFAUXCMD
\DIFdel{to two widely used gene set enrichment tools: single-sample gene set enrichment analysis (ssGSEA) and gene set variation analysis (GSVA) \mbox{%DIFAUXCMD
\cite{barbieSystematicRNAInterference2009, hanzelmannGSVAGeneSet2013, tarcaComparisonGeneSet2013}}\hspace{0pt}%DIFAUXCMD
. Both methods are implemented in the GSVA R package \mbox{%DIFAUXCMD
\cite{hanzelmannGSVAGeneSet2013}}\hspace{0pt}%DIFAUXCMD
...
%DIFDELCMD < %%%
\DIFdel{This approach allowed us to model cancer gene expression data while also controlling for subtype-related expression variation. We downloaded the RSEM-quantified TPM normalized gene expression measurements from the UCSC Xena Browser \mbox{%DIFAUXCMD
\cite{goldmanUCSCXenaPlatform2018}}\hspace{0pt}%DIFAUXCMD
. We focus our analysis on normalized gene expression data because this data is more widely used in the cancer research community and fewer methods are available to analyze normalized counts. To reduce heteroscedasticity and the effect of outlier expression levels, we transformed the expression data to log2(TPM + 1) \mbox{%DIFAUXCMD
\cite{zwienerTransformingRNASeqData2014}}\hspace{0pt}%DIFAUXCMD
.
}%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdel{We then applied the hydra framework using the hydra }\textit{\DIFdel{sweep}} %DIFAUXCMD
...
%DIF < \begin{figure}[!h]
%DIF < \caption{{\bf Bold the figure title.}
%DIF < Figure caption text here, please use this space for the figure panel %descriptions instead of using subfigure commands. A: Lorem ipsum dolor %sit amet. B: Consectetur adipiscing elit.}
%DIF < \label{fig1}
%DIF < \end{figure}
%DIFDELCMD < 
...
%DIFAUXCMD
\DIFdel{We downloaded pediatric cancer RNA-Seq data for neuroblastoma, osteosarcoma, Ewing sarcoma, alveolar rhabdomyosarcoma, and embryonal rhabdomyosarcoma from the UCSC Treehouse Compendium (}%DIFDELCMD < \url{https://treehousegenomics.soe.ucsc.edu/public-data/}%%%
\DIFdel{). This data was produced using the same RNA-seq pipeline, so potential computational batch effects are minimized \mbox{%DIFAUXCMD
\cite{vivianToilEnablesReproducible2017,vaskeComparativeTumorRNA2019}}\hspace{0pt}%DIFAUXCMD
. Clinical data for the TARGET neuroblastoma and osteosarcoma samples were obtained from the TARGET Data Matrix (https://ocg.cancer.gov/programs/target/data-matrix). We also analyzed a set of 58 synovial sarcoma microarray profiles with matching metastasis rate data \mbox{%DIFAUXCMD
\cite{lagardeChromosomeInstabilityAccounts2013}}\hspace{0pt}%DIFAUXCMD
.
}%DIFDELCMD < 
...
%DIFAUXCMD
\DIFdel{We applied each hydra tool to the TARGET }\textit{\DIFdel{MYCN}}%DIFAUXCMD
\DIFdel{-NA neuroblastoma cohort. We first obtained the multimodal gene models using the hydra }\textit{\DIFdel{filter}} %DIFAUXCMD
\DIFdel{tool. The hydra }\textit{\DIFdel{filter}} %DIFAUXCMD
...
%DIFDELCMD < %%%
\DIFdel{The hydra }\textit{\DIFdel{sweep}} %DIFAUXCMD
\DIFdel{command was applied to search for subtype expression within curated MSigDB gene sets. We included the hallmark (n=50), BioCarta (n=289), KEGG (n=186), PID (n=196), and Reactome (n=1499) genesets \mbox{%DIFAUXCMD
\cite{liberzonMolecularSignaturesDatabase2011}}\hspace{0pt}%DIFAUXCMD
...
%DIFDELCMD < %%%
\DIFdel{The hydra }\textit{\DIFdel{enrich}} %DIFAUXCMD
\DIFdel{command identified correlated expression signatures using the enriched GO term genes (FDR $<$ 0.01). The multivariate mixture model $\alpha$ concentration parameter was set to 5.0; the prior on the covariance matrix was set to the identity scaled by 2.0. The prior parameter for the number of clusters was set to 5. Our synthetic data analysis found that the signal decreases below an effect size of 1.0, so we use this parameter value for all following analyses. We used the hydra }\textit{\DIFdel{scan}} %DIFAUXCMD
...
%DIFDELCMD < %%%
\DIFdel{To validate tumor microenvironment expression subtypes, we correlated the hydra }\textit{\DIFdel{enrich}} %DIFAUXCMD
\DIFdel{expression clusters with the results of tumor microenvironment profiling tools xCell \mbox{%DIFAUXCMD
\cite{aranXCellDigitallyPortraying2017}}\hspace{0pt}%DIFAUXCMD
, CIBERSORT \mbox{%DIFAUXCMD
\cite{newmanRobustEnumerationCell2015}}\hspace{0pt}%DIFAUXCMD
, and ESTIMATE \mbox{%DIFAUXCMD
\cite{yoshiharaInferringTumourPurity2013a}}\hspace{0pt}%DIFAUXCMD
. We also compared the hydra }\textit{\DIFdel{enrich}} %DIFAUXCMD
\DIFdel{approach to state-of-the-art consensus clustering methods M3C \mbox{%DIFAUXCMD
\cite{johnM3CMonteCarlo2018} }\hspace{0pt}%DIFAUXCMD
and k-means clustering using the Gap statistic to select the number of clusters \mbox{%DIFAUXCMD
...
%DIFAUXCMD
\DIFdel{We then compared the clustering patterns across }\textit{\DIFdel{MYCN}}%DIFAUXCMD
\DIFdel{-NA neuroblastoma, osteosarcoma, Ewing sarcoma, embryonal rhabdomyosarcoma, alveolar rhabdomyosarcoma, and synovial sarcoma. We applied the TumorMap dimensionality reduction method \mbox{%DIFAUXCMD
\cite{newtonTumorMapExploringMolecular2017} }\hspace{0pt}%DIFAUXCMD
to visualize clustering of the full small blue round cell tumor gene expression matrix. We then applied the hydra framework to explore expression variation within each disease. Each disease expression matrix had unique statistical properties including sample size and subtype variation. This required us to adapt the minimum probability threshold for each disease dataset using the }\textit{\DIFdel{scan}} %DIFAUXCMD
\DIFdel{routine. The Jupyter notebooks for exploring these datasets can be found on GitHub (}%DIFDELCMD < \url{www.github.com/jpfeil/hydra-paper/analysis}%%%
...
%DIFAUXCMD
\DIFdel{A Kruskal-Wallis test was used to identify statistically significant differences across two or more groups, and a Mann-Whitney U test was used for pairwise tests using a Holm-Sidak correction for multiple hypothesis testing \mbox{%DIFAUXCMD
\cite{pedregosa2011scikit,jonesSciPyOpenSource2001}}\hspace{0pt}%DIFAUXCMD
. We used the scipy \mbox{%DIFAUXCMD
\cite{2019arXiv190710121V} }\hspace{0pt}%DIFAUXCMD
stats implementation of the Kruskal-Wallis test and the scikit-learn post hoc processing \mbox{%DIFAUXCMD
\cite{Terpilowski2019} }\hspace{0pt}%DIFAUXCMD
implementation of pairwise Mann-Whitney U tests. Spearman rank and Pearson correlation values were calculated using the scipy library \mbox{%DIFAUXCMD
\cite{jonesSciPyOpenSource2001}}\hspace{0pt}%DIFAUXCMD
. Survival analysis was done using the survminer package \mbox{%DIFAUXCMD
\cite{kassambaraSurvminerDrawingSurvival2019}}\hspace{0pt}%DIFAUXCMD
.
}%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdelend % Results and Discussion can be combined.
\section*{Results}
...
%DIF >  TODO Need to fix supplemental figure references
...
\DIFaddbegin \section*{\DIFadd{Materials and methods}}
\subsection*{\DIFadd{Dirichlet process gaussian mixture model}}
\DIFadd{Traditional parametric models, like the finite mixture model, use a fixed number of parameters (i.e. number of clusters). Over- or underfitting can occur when the parametric model does not reflect the underlying data \mbox{%DIFAUXCMD
\cite{teh2010dirichlet}}\hspace{0pt}%DIFAUXCMD
. Unlike the finite mixture model, the Dirichlet process mixture model (DPMM) represents a theoretically infinite number of clusters and can adapt the number of clusters based on prior belief and the data \mbox{%DIFAUXCMD
\cite{gelmanBayesianDataAnalysis2013, antoniakMixturesDirichletProcesses1974, teh2010dirichlet}}\hspace{0pt}%DIFAUXCMD
.
...
\DIFadd{The Dirichlet process ($DP$) is an infinite dimensional extension of the Dirichlet distribution \mbox{%DIFAUXCMD
\cite{fergusonBayesianAnalysisNonparametric1973} }\hspace{0pt}%DIFAUXCMD
and is commonly used as a prior distribution for infinite mixture models \mbox{%DIFAUXCMD
\cite{muller2004nonparametric, gorurDirichletProcessGaussian2010}}\hspace{0pt}%DIFAUXCMD
. The Dirichlet process has two parameters: the concentration parameter $\alpha$ and centering distribution $H$. The concentration parameter $\alpha$, where $\alpha \in \mathbb{R}^+$, controls the extent to which samples from the $DP$ resemble the centering distribution $H$. We model gene expression as a multivariate Gaussian distribution, so our centering distribution is a normal-Wishart distribution ($\mathcal{NW}_0$).
...
\DIFadd{We briefly describe the stick-breaking construction of the Dirichlet process $G \sim DP(\alpha, H)$. Consider a stick of unit length. To generate an infinite number of mixing weights $\pi_1, \pi_2, ..., \pi_k$ for the DPMM, first break a stick of unit length at $\nu \in [0, 1]$ where $\nu$ is sampled from a Beta distribution, and set $\pi_1$ to be the length of the first piece. We repeat this process using the remainder of the stick for each $\pi_k$. The DP is truncated to the number of clusters $K$, which was shown to accurately approximate the infinite posterior for large K \mbox{%DIFAUXCMD
\cite{hughes2013memoized, teh2010dirichlet, gelmanBayesianDataAnalysis2013, fergusonBayesianAnalysisNonparametric1973, mullerBayesianNonparametricData2015, phadia2015prior}}\hspace{0pt}%DIFAUXCMD
.
}
...
\DIFadd{Next, we sample the parameters from the centering distribution $H$ weighted by the mixing components. If we consider a probability space $\Theta$ where $\theta_k^{*} \in \Theta$, then $H$ is a measure on the partitions of $\Theta$. For our application, we will partition the parameter space $\Theta$ into finite, measurable partitions $B_1, B_2, ..., B_k$.
}

\begin{gather}
\DIFadd{\theta_k^{*} \sim H }\\
\DIFadd{G = \sum_{k=1}^{\infty} \pi_k \delta_{\theta_k^{*}} }\\
\DIFadd{(G(B_1), G(B_2), ..., G(B_k)) \sim \text{Dir}(\alpha H(B_1), \alpha H(B_2), ..., \alpha H(B_k))
}\end{gather}

\DIFadd{This construction generates the marginal of the Dirichlet process, which follows a Dirichlet distribution. Samples from the marginal distribution are finite, discrete, and sum to $1$ \mbox{%DIFAUXCMD
\cite{fergusonBayesianAnalysisNonparametric1973}}\hspace{0pt}%DIFAUXCMD
. Next, we outline how the DPMM groups gene expression samples $x_i$ under cluster-specific parameters $\mu_{z_i}$ and $\Sigma_{z_i}$ where $z_i \in {1, 2, ..., K}$ is the cluster index.
}
...
\DIFadd{\label{eq:mm}
x_i | \mu_{z_i}, \Sigma_{z_i} \sim \mathcal{N}(\mu_{z_i}, \Sigma_{z_i}) }\\
\DIFadd{z_i | \pi \sim \text{Categorical}(\pi_1, \pi_2, ..., \pi_k) }\\
\DIFadd{\mu_{z_i}, \Sigma_{z_i} | G \sim G }\\
\DIFadd{G | \alpha, \mathcal{NW}_0 \sim DP(\alpha, \mathcal{NW}_0)
}\end{gather}

\DIFadd{To improve our methods ability to scale to larger datasets, we incorporated the bnpy memoized online variational inference algorithm (moVB) \mbox{%DIFAUXCMD
\cite{hughes2013memoized} }\hspace{0pt}%DIFAUXCMD
into our analysis framework. The moVB algorithm uses variational inference to approximate the posterior distribution and interleaves birth, merge, and delete moves to avoid local optima and remove redundant clusters \mbox{%DIFAUXCMD
\cite{hughesBnpyReliableScalable}}\hspace{0pt}%DIFAUXCMD
...
\subsection*{\DIFadd{Hydra method}}
\DIFadd{We developed a Bayesian non-parametric clustering framework for identifying biological and technical variation in large cancer gene expression datasets without the need for a reference normal dataset. To our knowledge, this is the first reproducible and widely deployable implementation of a non-parametric mixture model framework designed to overcome the challenges of precision oncology gene expression analysis. The hydra pipeline is an open source software tool hosted on GitHub (}\url{www.github.com/jpfeil/hydra}\DIFadd{). A Docker container is available for deployment across environments (}\url{https://hub.docker.com/r/jpfeil/hydra}\DIFadd{).
}

\DIFadd{The hydra framework contains three main command-line tools: }\textit{\DIFadd{filter}}\DIFadd{, }\textit{\DIFadd{enrich}}\DIFadd{, and }\textit{\DIFadd{sweep}} \DIFadd{(Fig \ref{overview}). The }\textit{\DIFadd{filter}} \DIFadd{command is run first to isolate the multimodally expressed genes using a univariate Dirichlet Process Gaussian Mixture Model (DP-GMM). There are two methods for analyzing the resulting set of multimodally expressed genes. The }\textit{\DIFadd{enrich}} \DIFadd{method, which subsets to the genes found to be significantly enriched in biological pathways, and the }\textit{\DIFadd{sweep}} \DIFadd{method, which searches within user-defined gene sets for multimodal expression signatures. The underlying analysis routines can be accessed within the Docker using Jupyter notebooks to facilitate the development of user-defined workflows.
}

%DIF >  Overview Figure
\begin{figure}[h!]
	%DIF > \includegraphics[width=\textwidth]{"img/PNG/overview-fig"}
	\caption{{\bf \DIFaddFL{Overview of the hydra framework tools.}}
		\DIFaddFL{A: Suggested workflow for applying hydra framework tools to identify clinically relevant gene expression subtypes. B: The hydra }\textit{\DIFaddFL{filter}} \DIFaddFL{command removes unimodally distributed genes which greatly reduces the number of genes in downstream clustering analysis. C: The hydra }\textit{\DIFaddFL{enrich}} \DIFaddFL{command takes the multimodally expressed genes and returns enriched gene sets. The enriched gene set genes are used for multivariate clustering of samples. D: The hydra
		}\textit{\DIFaddFL{sweep}} \DIFaddFL{command looks for multivariate normal clusters within user-defined gene sets. This can be used for the automatic detection of clusters in large gene set databases. Abbreviation: Tumor microenvironment (TME) 
		}\label{overview}}
\end{figure}

\DIFadd{The }\textit{\DIFadd{filter}} \DIFadd{command (Fig \ref{overview}B) takes an expression matrix and filters the genes down to the multimodally expressed genes using the DP-GMM described above. We apply a DP-GMM to each gene, saving the model for genes with two or more expression clusters. This creates a directory of multimodally expressed gene models which can be used to predict differential expression in new samples. This analysis framework is a novel contribution to the precision medicine research community. Our approach has several beneficial properties. For example, training models on curated data sets and applying the models to new samples avoids the use of reference distributions, which overestimate the uncertainty in the analysis by not accounting for subtype expression. Furthermore, this approach identifies the set of most strongly differentially expressed genes within a disease context, which may enrich for potential biomarkers for precision medicine applications. The multimodally expressed genes are also used in downstream clustering analysis.
}

\DIFadd{The }\textit{\DIFadd{enrich}} \DIFadd{(Fig \ref{overview}C) and }\textit{\DIFadd{sweep}} \DIFadd{(Fig \ref{overview}D) routines are two independent analyses to explore multimodal expression in cancer gene expression cohorts. In addition to identifying expression variation within a disease context, we also found that multimodally expressed genes that participate in a biological pathway tend to have correlated expression distributions. This insight facilitates the detection of multimodal expression signatures by enriching for genes that have multimodal expression distributions and participate in known biological processes. The hydra software comes prepackaged with popular gene sets, including the Molecular Signatures Database (MSigDB) \mbox{%DIFAUXCMD
\cite{liberzonMolecularSignaturesDatabase2011}}\hspace{0pt}%DIFAUXCMD
, the Gene Ontology terms \mbox{%DIFAUXCMD
\cite{Ashburner2000, gene2018gene}}\hspace{0pt}%DIFAUXCMD
, and the EnrichmentMap gene sets \mbox{%DIFAUXCMD
...
\DIFadd{The }\textit{\DIFadd{enrich}} \DIFadd{command uses a hypergeometric test \mbox{%DIFAUXCMD
\cite{yuClusterProfilerPackageComparing2012} }\hspace{0pt}%DIFAUXCMD
to discover enrichment of multimodally expressed genes within a user-defined database of gene sets. This creates a list of gene sets and a list of enriched gene set genes. The }\textit{\DIFadd{enrich}} \DIFadd{method outputs a table of enriched gene sets while also clustering samples across the genes that participate in the enriched gene sets. The table of enriched gene sets may reveal surprising expression patterns and generate hypotheses for further investigation of tumor subtypes.
}

\DIFadd{The implementation of the }\textit{\DIFadd{enrich}} \DIFadd{method includes an important parameter known as the minimum component probability. The minimum component probability is the probability of placing a sample within the smallest expression cluster. This is an additional filter to remove multimodally expressed genes that influence a relatively small subset of tumor samples. This parameter gives the user the ability to subset the enriched genes to those that influence a greater number of patients. To aid in the exploration of minimum component thresholds, we implemented a }\textit{\DIFadd{scan}} \DIFadd{sub-routine. The }\textit{\DIFadd{scan}} \DIFadd{routine tunes the analysis with respect to the constraints of the available data (e.g. number of samples and number of genes), which is an important factor in pediatric cancer research since data is often difficult to obtain and so datasets are relatively small. We recommend setting this threshold such that the number of genes is less than the number of samples because otherwise the inference may become unstable \mbox{%DIFAUXCMD
\cite{caiConstrainedL1Minimization2011}}\hspace{0pt}%DIFAUXCMD
.
}

\DIFadd{The }\textit{\DIFadd{sweep}} \DIFadd{routine identifies differentially expressed gene sets and can be used as an alternative to single-sample GSEA \mbox{%DIFAUXCMD
\cite{subramanianGeneSetEnrichment2005}}\hspace{0pt}%DIFAUXCMD
. For each gene set, a multivariate DP-GMM is applied to determine if more than one expression cluster is present within the gene set. This approach is useful when curated gene sets are available for the disease of interest, but manual inspection of each gene set is not feasible. Reducing the genes to multimodally expressed genes facilitates the detection of differentially expressed gene sets. Existing gene set enrichment tools are known to under-perform when the expression is correlated \mbox{%DIFAUXCMD
\cite{tamayoLimitationsSimpleGene2016}}\hspace{0pt}%DIFAUXCMD
, but our approach is designed to identify distinct correlation structures within gene expression datasets.
...
\DIFadd{We have also implemented routines for cluster profiling and N-of-1 tumor analysis. These routines are accessible within the docker container using the Jupyter notebook command. Cluster profiling analysis of clusters derived from the }\textit{\DIFadd{enrich}} \DIFadd{or }\textit{\DIFadd{sweep}} \DIFadd{routines includes GSEA \mbox{%DIFAUXCMD
\cite{korotkevichFastGeneSet2019} }\hspace{0pt}%DIFAUXCMD
to identify the pathway expression that characterizes each cluster. GSEA uses all available genes since it requires non-differentially expressed genes to assess the significance of an enrichment score. A t-statistic is calculated for each gene, comparing gene expression values of samples inside to those outside of a cluster. Cluster profiling GSEA uses the ranked gene-level t-statistics to determine gene set enrichment.
}

\DIFadd{The N-of-1 tumor analysis routine classifies a new gene expression profile into one of the inferred clusters, calculates a gene-level z-score for that sample relative to the normalized expression distribution, and performs standard GSEA using a preranked list of z-score values \mbox{%DIFAUXCMD
\cite{korotkevichFastGeneSet2019}}\hspace{0pt}%DIFAUXCMD
. This procedure can identify new gene expression signatures that may not be detectable using the entire expression cohort as a background reference distribution. This approach is another novel contribution to the field and may facilitate the identification of clinically relevant signatures that are being overlooked in current gene expression analyses.
}

\subsection*{\DIFadd{Synthetic data generation and validation}}
%DIF >  For figure citations, please use "Fig" instead of "Figure".
\DIFadd{We first tested the hydra framework’s ability to detect differential pathway expression using synthetic cancer data. We compared hydra }\textit{\DIFadd{sweep}} \DIFadd{to two widely used gene set enrichment tools: single-sample gene set enrichment analysis (ssGSEA) and gene set variation analysis (GSVA) \mbox{%DIFAUXCMD
\cite{barbieSystematicRNAInterference2009, hanzelmannGSVAGeneSet2013, tarcaComparisonGeneSet2013}}\hspace{0pt}%DIFAUXCMD
. Both methods are implemented in the GSVA R package \mbox{%DIFAUXCMD
\cite{hanzelmannGSVAGeneSet2013}}\hspace{0pt}%DIFAUXCMD
. In order to accurately model correlation structures within cancer cohorts, we modeled the synthetic cancer gene expression data as a multivariate Gaussian distribution. We used the TCGA glioblastoma multiforme (GBM) cohort (N=166) to model a background mean and covariance matrix for the synthetic data analysis. We chose TCGA GBM, a very different disease from those analyzed in the remainder of this manuscript, to avoid overfitting the hydra method to diseases of interest. This also enables us to demonstrate the flexibility of our method to analyze data from a variety of cancer genome sequencing projects.
...
\DIFadd{This approach allowed us to model cancer gene expression data while also controlling for subtype-related expression variation. We downloaded the RSEM-quantified transcripts per million (TPM) normalized gene expression measurements from the UCSC Xena Browser \mbox{%DIFAUXCMD
\cite{goldmanUCSCXenaPlatform2018}}\hspace{0pt}%DIFAUXCMD
. We focus our analysis on normalized gene expression data because this data is more widely used in the cancer research community and fewer methods are available to analyze normalized counts. To reduce heteroscedasticity and the effect of outlier expression levels, we transformed the expression data to log2(TPM + 1) \mbox{%DIFAUXCMD
\cite{zwienerTransformingRNASeqData2014}}\hspace{0pt}%DIFAUXCMD
.
...
\DIFadd{We then applied the hydra framework using the hydra }\textit{\DIFadd{sweep}} \DIFadd{command (Fig \ref{overview}C), since this method is directly comparable to the single-sample GSEA methods. The mean expression filter removed any genes with a mean expression of fewer than 1.0 log2(TPM + 1). This avoids lowly-expressed genes that may have particularly noisy expression measurements. The prior on the hydra covariance matrix was the identity scaled by 2.0 and the prior on the number of clusters was set to 2 because we expect there to be an activated cluster and a baseline expression cluster. We set the over-expressing cluster to be the cluster with the largest L1 norm.
}
...
\subsection*{\DIFadd{Pediatric cancer gene expression data}}
\DIFadd{We downloaded pediatric cancer RNA-Seq data for neuroblastoma, osteosarcoma, Ewing sarcoma, alveolar rhabdomyosarcoma, and embryonal rhabdomyosarcoma from the UCSC Treehouse Compendium (}\url{https://treehousegenomics.soe.ucsc.edu/public-data/}\DIFadd{). This data was produced using the same RNA-seq pipeline, so potential computational batch effects are minimized \mbox{%DIFAUXCMD
\cite{vivianToilEnablesReproducible2017,vaskeComparativeTumorRNA2019}}\hspace{0pt}%DIFAUXCMD
. Clinical data for the TARGET neuroblastoma and osteosarcoma samples were obtained from the TARGET Data Matrix (https://ocg.cancer.gov/programs/target/data-matrix). We also analyzed a set of 58 synovial sarcoma microarray profiles with matching metastasis rate data \mbox{%DIFAUXCMD
\cite{lagardeChromosomeInstabilityAccounts2013}}\hspace{0pt}%DIFAUXCMD
.
...
\subsection*{\DIFadd{TARGET neuroblastoma analysis}}
\DIFadd{We applied each hydra tool to the TARGET }\textit{\DIFadd{MYCN}}\DIFadd{-non-amplified neuroblastoma cohort. We first obtained the multimodal gene models using the hydra }\textit{\DIFadd{filter}} \DIFadd{tool. The hydra }\textit{\DIFadd{filter}} \DIFadd{tool identified all genes with a multimodal expression pattern. We used the mean expression filter to remove genes that may have unstable measurements due to low transcript abundances. We excluded all genes with a mean expression value less than 1 log2(TPM + 1).
}
...
\DIFadd{The hydra }\textit{\DIFadd{enrich}} \DIFadd{command identified correlated expression signatures using the enriched GO term genes (FDR $<$ 0.01). The multivariate mixture model $\alpha$ concentration parameter was set to 5.0; the prior on the covariance matrix was set to the identity scaled by 2.0. The prior parameter for the number of clusters was set to 5. Our synthetic data analysis found that the signal decreases below an effect size of 1.0, so we use this parameter value for all following analyses. We used the hydra }\textit{\DIFadd{scan}} \DIFadd{routine to search a range of minimum component probability thresholds (see Results) and found that a threshold/probability of 20\% yielded the most clusters while keeping the number of genes (p = 42) below the number of samples (n = 70).
}

\DIFadd{To validate tumor microenvironment expression subtypes, we correlated the hydra }\textit{\DIFadd{enrich}} \DIFadd{expression clusters with the results of tumor microenvironment profiling tools xCell \mbox{%DIFAUXCMD
\cite{aranXCellDigitallyPortraying2017}}\hspace{0pt}%DIFAUXCMD
, CIBERSORT \mbox{%DIFAUXCMD
\cite{newmanRobustEnumerationCell2015}}\hspace{0pt}%DIFAUXCMD
, and ESTIMATE \mbox{%DIFAUXCMD
\cite{yoshiharaInferringTumourPurity2013a}}\hspace{0pt}%DIFAUXCMD
. We also compared the hydra }\textit{\DIFadd{enrich}} \DIFadd{approach to state-of-the-art consensus clustering methods M3C \mbox{%DIFAUXCMD
\cite{johnM3CMonteCarlo2018} }\hspace{0pt}%DIFAUXCMD
and k-means clustering using the Gap statistic to select the number of clusters \mbox{%DIFAUXCMD
\cite{tibshirani2001estimating}}\hspace{0pt}%DIFAUXCMD
. Since these methods are influenced by the number of input genes, we tested a range of median absolute deviation (MAD) thresholds. The number of clusters was assumed to be the smallest statistically significant value.
...
\subsection*{\DIFadd{Small blue round cell tumor analysis}}
\DIFadd{We then compared the clustering patterns across }\textit{\DIFadd{MYCN}}\DIFadd{-NA neuroblastoma, osteosarcoma, Ewing sarcoma, embryonal rhabdomyosarcoma, alveolar rhabdomyosarcoma, and synovial sarcoma. We applied the TumorMap dimensionality reduction method \mbox{%DIFAUXCMD
\cite{newtonTumorMapExploringMolecular2017} }\hspace{0pt}%DIFAUXCMD
to visualize clustering of the full small blue round cell tumor gene expression matrix. We then applied the hydra framework to explore expression variation within each disease. Each disease expression matrix had unique statistical properties including sample size and subtype variation. This required us to adapt the minimum probability threshold for each disease dataset using the }\textit{\DIFadd{scan}} \DIFadd{routine. The Jupyter notebooks for exploring these datasets can be found on GitHub (}\url{www.github.com/jpfeil/hydra-paper/analysis}\DIFadd{). We used agglomerative clustering to investigate patterns in the top 10 enriched gene sets for each disease's expression subtypes.
}

\subsection*{\DIFadd{Statistical analysis}}
\DIFadd{A Kruskal-Wallis test was used to identify statistically significant differences across two or more groups, and a Mann-Whitney U test was used for pairwise tests using a Holm-Sidak correction for multiple hypothesis testing \mbox{%DIFAUXCMD
\cite{pedregosa2011scikit,jonesSciPyOpenSource2001}}\hspace{0pt}%DIFAUXCMD
. We used the scipy \mbox{%DIFAUXCMD
\cite{2019arXiv190710121V} }\hspace{0pt}%DIFAUXCMD
stats implementation of the Kruskal-Wallis test and the scikit-learn post hoc processing \mbox{%DIFAUXCMD
...
\subsection*{\DIFadd{H\&E slide preparation and pathologist review}}
\DIFadd{Pediatric tumor samples were flash frozen, embedded in OCT, and 5$\mu$m cryosections were collected. Slides were hematoxylin and eosin (H\&E) stained and imaged on a Leica DMi8, equipped with a HC PL APO 40x/0.85 NA objective and DFC7000T camera. H\&E slides were reviewed by a licensed pathologist.  Morphologic analysis was performed and the degree and type of inflammation estimated from the histologic sections. Grading of inflammation was either minimal ($<$10\% of total nuclei consist of inflammatory cells) or  moderate (20-30\% of total nuclei consist of inflammatory cells). The type of inflammation (predominantly small mature lymphocytes or mixed inflammation consisting of small mature lymphocytes along with plasma cells and/or eosinophils) was noted for each tumor sample.
}
...
%DIF < \bibliographystyle{plos2015.bst}
\DIFaddbegin \bibliographystyle{plos2015.bst}
\DIFaddend %\bibliography{zotero-library}
...
%DIFDELCMD < 	%%%
\DIFdelend \bibitem{vivianToilEnablesReproducible2017}
	...
	Dahl DB\DIFdelbegin \DIFdel{, Vannucci M}\DIFdelend .
...
\DIFdelend \DIFaddbegin \DIFadd{Model-Based Clustering for Expression Data }\DIFaddend via a {{Dirichlet\DIFaddbegin }}
	\DIFaddend Process Mixture Model\DIFdelbegin %DIFDELCMD < }}%%%
\DIFdel{.
	}%DIFDELCMD < \newblock %%%
\DIFdel{In: Do KA, Muller P,editors. Bayesian }\DIFdelend \DIFaddbegin \DIFadd{,'' in }\DIFaddend {{\DIFaddbegin \DIFadd{Bayesian }\DIFaddend Inference}} for {{Gene Expression}}
	and {{Proteomics}}\DIFdelbegin \DIFdel{. }%DIFDELCMD < {%%%
\DIFdel{Cambridge}%DIFDELCMD < }%%%
\DIFdel{: }%DIFDELCMD < {%%%
\DIFdel{Cambridge University Press}%DIFDELCMD < }%%%
\DIFdelend ; 2006.
	\DIFdelbegin \DIFdel{p. 201--218.
	}\DIFdelend 
	...
	\DIFaddbegin \bibitem{morgensternChallengeDefiningUltrahighrisk2019}
	\DIFadd{Morgenstern DA, Bagatell R, Cohn SL, Hogarty MD, Maris JM, Moreno L, et~al.
	}\newblock \DIFadd{The Challenge of Defining ``Ultra-High-Risk'' Neuroblastoma.
	}\newblock \DIFadd{Pediatric Blood \& Cancer. 2019;66(4):e27556.
	}\newblock \DIFadd{doi:}{\DIFadd{10.1002/pbc.27556}}\DIFadd{.
	}
	...
	\DIFadd{Foster DS, Jones RE, Ransom RC, Longaker MT, Norton JA.
	}\newblock \DIFadd{The Evolving Relationship of Wound Healing and Tumor Stroma.
	}\newblock \DIFadd{JCI Insight;3(18).
	}\newblock \DIFadd{doi:}{\DIFadd{10.1172/jci.insight.99911}}\DIFadd{.
	}

	\bibitem{aranXCellDigitallyPortraying2017}
	\DIFadd{Aran D, Hu Z, Butte AJ.
	}\newblock {{\DIFadd{xCell}}}\DIFadd{: Digitally Portraying the Tissue Cellular Heterogeneity
	Landscape.
	}\newblock \DIFadd{Genome Biology. 2017;18(1):220.
	}\newblock \DIFadd{doi:}{\DIFadd{10.1186/s13059-017-1349-1}}\DIFadd{.
	}

	\bibitem{yoshiharaInferringTumourPurity2013a}
	\DIFadd{Yoshihara K, Shahmoradgoli M, Mart}{\DIFadd{\'i}}\DIFadd{nez E, Vegesna R, Kim H, }{\DIFadd{Torres-Garcia}}
	\DIFadd{W, et~al.
	}\newblock \DIFadd{Inferring Tumour Purity and Stromal and Immune Cell Admixture from
	Expression Data.
	}\newblock \DIFadd{Nature Communications. 2013;4:2612.
	}\newblock \DIFadd{doi:}{\DIFadd{10.1038/ncomms3612}}\DIFadd{.
	}

	\bibitem{bourgonIndependentFilteringIncreases2010}
	\DIFadd{Bourgon R, Gentleman R, Huber W.
	}\newblock \DIFadd{Independent Filtering Increases Detection Power for High-Throughput
	Experiments.
	}\newblock \DIFadd{Proceedings of the National Academy of Sciences.
	2010;107(21):9546--9551.
	}\newblock \DIFadd{doi:}{\DIFadd{10.1073/pnas.0914005107}}\DIFadd{.
	}

	\bibitem{tritchlerFilteringGenesCluster2009}
	\DIFadd{Tritchler D, Parkhomenko E, Beyene J.
	}\newblock \DIFadd{Filtering }{{\DIFadd{Genes}}} \DIFadd{for }{{\DIFadd{Cluster}}} \DIFadd{and }{{\DIFadd{Network Analysis}}}\DIFadd{.
	}\newblock \DIFadd{BMC Bioinformatics. 2009;10(1):193.
	}\newblock \DIFadd{doi:}{\DIFadd{10.1186/1471-2105-10-193}}\DIFadd{.
	}

	\bibitem{carcamo-oriveAnalysisTranscriptionalVariability2017}
	{\DIFadd{Carcamo-Orive}} \DIFadd{I, Hoffman GE, Cundiff P, Beckmann ND, D'Souza SL, Knowles JW,
	et~al.
	}\newblock \DIFadd{Analysis of }{{\DIFadd{Transcriptional Variability}}} \DIFadd{in a }{{\DIFadd{Large Human iPSC
			Library Reveals Genetic}}} \DIFadd{and }{{\DIFadd{Non}}}\DIFadd{-Genetic }{{\DIFadd{Determinants}}} \DIFadd{of
	}{{\DIFadd{Heterogeneity}}}\DIFadd{.
	}\newblock \DIFadd{Cell Stem Cell. 2017;20(4):518--532.e9.
	}\newblock \DIFadd{doi:}{\DIFadd{10.1016/j.stem.2016.11.005}}\DIFadd{.
	}

	\bibitem{tibshirani2001estimating}
	\DIFadd{Tibshirani R, Walther G, Hastie T.
	}\newblock \DIFadd{Estimating the Number of Clusters in a Data Set via the Gap
	Statistic.
	...
	\DIFadd{Maechler M, Rousseeuw P, Struyf A, Hubert M, Hornik K, et~al.
	}\newblock \DIFadd{Cluster: Cluster Analysis Basics and Extensions.
	}\newblock \DIFadd{R package version. 2012;1(2):56.
	}

	\bibitem{reevesAntigenProcessingImmune2017}
	\DIFadd{Reeves E, James E.
	}\newblock \DIFadd{Antigen Processing and Immune Regulation in the Response to Tumours.
	}\newblock \DIFadd{Immunology. 2017;150(1):16--24.
	}\newblock \DIFadd{doi:}{\DIFadd{10.1111/imm.12675}}\DIFadd{.
	}

	\bibitem{hermansJAK1JAK2Inhibitor2018}
	\DIFadd{Hermans MAW, Schrijver B, }{\DIFadd{van Holten-Neelen}} \DIFadd{CCPA, }{\DIFadd{Gerth van Wijk}} \DIFadd{R, }{\DIFadd{van
		Hagen}} \DIFadd{PM, }{\DIFadd{van Daele}} \DIFadd{PLA, et~al.
	}\newblock \DIFadd{The }{{\DIFadd{JAK1}}}\DIFadd{/}{{\DIFadd{JAK2}}}\DIFadd{- Inhibitor Ruxolitinib Inhibits Mast Cell
	Degranulation and Cytokine Release.
	}\newblock \DIFadd{Clinical and Experimental Allergy: Journal of the British Society for
	Allergy and Clinical Immunology. 2018;48(11):1412--1420.
	}\newblock \DIFadd{doi:}{\DIFadd{10.1111/cea.13217}}\DIFadd{.
	}
	...
	\DIFadd{Hoadley KA, Yau C, Hinoue T, Wolf DM, Lazar AJ, Drill E, et~al.
	}\newblock \DIFadd{Cell-of-}{{\DIFadd{Origin Patterns Dominate}}} \DIFadd{the }{{\DIFadd{Molecular Classification}}}
	\DIFadd{of 10,000 }{{\DIFadd{Tumors}}} \DIFadd{from 33 }{{\DIFadd{Types}}} \DIFadd{of }{{\DIFadd{Cancer}}}\DIFadd{.
	}\newblock \DIFadd{Cell. 2018;173(2):291--304.e6.
	}\newblock \DIFadd{doi:}{\DIFadd{10.1016/j.cell.2018.03.022}}\DIFadd{.
	}

	\bibitem{cunananEfficientBasketTrial2017}
	\DIFadd{Cunanan KM, Iasonos A, Shen R, Begg CB, G}{\DIFadd{\"o}}\DIFadd{nen M.
	}\newblock \DIFadd{An }{{\DIFadd{Efficient Basket Trial Design}}}\DIFadd{.
	}\newblock \DIFadd{Statistics in medicine. 2017;36(10):1568--1579.
	}\newblock \DIFadd{doi:}{\DIFadd{10.1002/sim.7227}}\DIFadd{.
	}

	\bibitem{rheeImpactTumorPurity2018}
	\DIFadd{Rhee JK, Jung YC, Kim KR, Yoo J, Kim J, Lee YJ, et~al.
	}\newblock \DIFadd{Impact of }{{\DIFadd{Tumor Purity}}} \DIFadd{on }{{\DIFadd{Immune Gene Expression}}} \DIFadd{and
	}{{\DIFadd{Clustering Analyses}}} \DIFadd{across }{{\DIFadd{Multiple Cancer Types}}}\DIFadd{.
	}\newblock \DIFadd{Cancer Immunology Research. 2018;6(1):87--97.
	}\newblock \DIFadd{doi:}{\DIFadd{10.1158/2326-6066.CIR-17-0201}}\DIFadd{.
	}

	\bibitem{raphael2017integrated}
	\DIFadd{Raphael BJ, Hruban RH, Aguirre AJ, Moffitt RA, Yeh JJ, Stewart C, et~al.
	}\newblock \DIFadd{Integrated Genomic Characterization of Pancreatic Ductal
	Adenocarcinoma.
	}\newblock \DIFadd{Cancer cell. 2017;32(2):185--203.
	}

	\DIFaddend \bibitem{teh2010dirichlet}
	Teh YW.
	...
	\bibitem{caiConstrainedL1Minimization2011}
	Cai T, Liu W, Luo X.
	...
	\DIFdelbegin %DIFDELCMD < \bibitem{aranXCellDigitallyPortraying2017}
%DIFDELCMD < 	%%%
\DIFdel{Aran D, Hu Z, Butte AJ.
	}%DIFDELCMD < \newblock {{%%%
\DIFdel{xCell}%DIFDELCMD < }}%%%
...
\DIFdel{10.1186/s13059-017-1349-1}%DIFDELCMD < }%%%
\DIFdel{.
	}%DIFDELCMD < 

%DIFDELCMD < 	%%%
\DIFdelend \bibitem{newmanRobustEnumerationCell2015}
	...
	\DIFdelbegin %DIFDELCMD < \bibitem{yoshiharaInferringTumourPurity2013a}
%DIFDELCMD < 	%%%
\DIFdel{Yoshihara K, Shahmoradgoli M, Mart}%DIFDELCMD < {%%%
\DIFdel{\'i}%DIFDELCMD < }%%%
\DIFdel{nez E, Vegesna R, Kim H, }%DIFDELCMD < {%%%
\DIFdel{Torres-Garcia}%DIFDELCMD < }
%DIFDELCMD < 	%%%
\DIFdel{W, et~al.
	}%DIFDELCMD < \newblock %%%
...
\DIFdel{doi:}%DIFDELCMD < {%%%
\DIFdel{10.1038/ncomms3612}%DIFDELCMD < }%%%
\DIFdel{.
	}%DIFDELCMD < 

%DIFDELCMD < 	\bibitem{tibshirani2001estimating}
%DIFDELCMD < 	%%%
...
%DIFDELCMD < 	%%%
\DIFdelend \bibitem{pedregosa2011scikit}
...
%DIFDELCMD < 	\bibitem{morgensternChallengeDefiningUltrahighrisk2019}
%DIFDELCMD < 	%%%
...
\DIFdel{doi:}%DIFDELCMD < {%%%
\DIFdel{10.1002/pbc.27556}%DIFDELCMD < }%%%
\DIFdel{.
	}%DIFDELCMD < 

%DIFDELCMD < 	\bibitem{cotto2017dgidb}
%DIFDELCMD < 	%%%
\DIFdel{Cotto KC, Wagner AH, Feng YY, Kiwala S, Coffman AC, Spies G, et~al.
	}%DIFDELCMD < \newblock {{%%%
\DIFdel{DGIdb}%DIFDELCMD < }} %%%
...
%DIFDELCMD < 	\bibitem{fosterEvolvingRelationshipWound}
%DIFDELCMD < 	%%%
...
\DIFdel{doi:}%DIFDELCMD < {%%%
\DIFdel{10.1172/jci.insight.99911}%DIFDELCMD < }%%%
\DIFdel{.
	}%DIFDELCMD < 

%DIFDELCMD < 	\bibitem{bourgonIndependentFilteringIncreases2010}
%DIFDELCMD < 	%%%
...
\DIFdel{doi:}%DIFDELCMD < {%%%
\DIFdel{10.1073/pnas.0914005107}%DIFDELCMD < }%%%
\DIFdel{.
	}%DIFDELCMD < 

%DIFDELCMD < 	\bibitem{tritchlerFilteringGenesCluster2009}
%DIFDELCMD < 	%%%
\DIFdel{Tritchler D, Parkhomenko E, Beyene J.
	}%DIFDELCMD < \newblock %%%
\DIFdel{Filtering }%DIFDELCMD < {{%%%
\DIFdel{Genes}%DIFDELCMD < }} %%%
\DIFdel{for }%DIFDELCMD < {{%%%
\DIFdel{Cluster}%DIFDELCMD < }} %%%
\DIFdel{and }%DIFDELCMD < {{%%%
\DIFdel{Network Analysis}%DIFDELCMD < }}%%%
\DIFdel{.
	}%DIFDELCMD < \newblock %%%
\DIFdel{BMC Bioinformatics. 2009;10(1):193.
	}%DIFDELCMD < \newblock %%%
\DIFdel{doi:}%DIFDELCMD < {%%%
\DIFdel{10.1186/1471-2105-10-193}%DIFDELCMD < }%%%
\DIFdel{.
	}%DIFDELCMD < 

%DIFDELCMD < 	\bibitem{carcamo-oriveAnalysisTranscriptionalVariability2017}
%DIFDELCMD < 	{%%%
\DIFdel{Carcamo-Orive}%DIFDELCMD < } %%%
\DIFdel{I, Hoffman GE, Cundiff P, Beckmann ND, D'Souza SL, Knowles JW,
	et~al.
	}%DIFDELCMD < \newblock %%%
\DIFdel{Analysis of }%DIFDELCMD < {{%%%
\DIFdel{Transcriptional Variability}%DIFDELCMD < }} %%%
\DIFdel{in a }%DIFDELCMD < {{%%%
\DIFdel{Large Human iPSC
			Library Reveals Genetic}%DIFDELCMD < }} %%%
\DIFdel{and }%DIFDELCMD < {{%%%
\DIFdel{Non}%DIFDELCMD < }}%%%
\DIFdel{-Genetic }%DIFDELCMD < {{%%%
\DIFdel{Determinants}%DIFDELCMD < }} %%%
\DIFdel{of
	}%DIFDELCMD < {{%%%
\DIFdel{Heterogeneity}%DIFDELCMD < }}%%%
\DIFdel{.
	}%DIFDELCMD < \newblock %%%
\DIFdel{Cell Stem Cell. 2017;20(4):518--532.e9.
	}%DIFDELCMD < \newblock %%%
\DIFdel{doi:}%DIFDELCMD < {%%%
\DIFdel{10.1016/j.stem.2016.11.005}%DIFDELCMD < }%%%
\DIFdel{.
	}%DIFDELCMD < 

%DIFDELCMD < 	\bibitem{maechler2012cluster}
%DIFDELCMD < 	%%%
...
%DIFDELCMD < 	\bibitem{reevesAntigenProcessingImmune2017}
%DIFDELCMD < 	%%%
...
\DIFdel{doi:}%DIFDELCMD < {%%%
\DIFdel{10.1111/imm.12675}%DIFDELCMD < }%%%
\DIFdel{.
	}%DIFDELCMD < 

%DIFDELCMD < 	\bibitem{hermansJAK1JAK2Inhibitor2018}
%DIFDELCMD < 	%%%
\DIFdel{Hermans MAW, Schrijver B, }%DIFDELCMD < {%%%
\DIFdel{van Holten-Neelen}%DIFDELCMD < } %%%
\DIFdel{CCPA, }%DIFDELCMD < {%%%
\DIFdel{Gerth van Wijk}%DIFDELCMD < } %%%
\DIFdel{R, }%DIFDELCMD < {%%%
\DIFdel{van
		Hagen}%DIFDELCMD < } %%%
\DIFdel{PM, }%DIFDELCMD < {%%%
...
\DIFdel{JAK1}%DIFDELCMD < }}%%%
\DIFdel{/}%DIFDELCMD < {{%%%
\DIFdel{JAK2}%DIFDELCMD < }}%%%
...
\DIFdel{doi:}%DIFDELCMD < {%%%
\DIFdel{10.1111/cea.13217}%DIFDELCMD < }%%%
\DIFdel{.
	}%DIFDELCMD < 

%DIFDELCMD < 	\bibitem{marisInitialTestingAurora2010}
%DIFDELCMD < 	%%%
\DIFdel{Maris JM, Morton CL, Gorlick R, Kolb EA, Lock R, Carol H, et~al.
	}%DIFDELCMD < \newblock %%%
\DIFdel{Initial Testing of the Aurora Kinase a Inhibitor }%DIFDELCMD < {{%%%
\DIFdel{MLN8237}%DIFDELCMD < }} %%%
\DIFdel{by the
	}%DIFDELCMD < {{%%%
\DIFdel{Pediatric Preclinical Testing Program}%DIFDELCMD < }} %%%
\DIFdel{(}%DIFDELCMD < {{%%%
\DIFdel{PPTP}%DIFDELCMD < }}%%%
\DIFdel{).
	}%DIFDELCMD < \newblock %%%
\DIFdel{Pediatric Blood \& Cancer. 2010;55(1):26--34.
	}%DIFDELCMD < \newblock %%%
\DIFdel{doi:}%DIFDELCMD < {%%%
\DIFdel{10.1002/pbc.22430}%DIFDELCMD < }%%%
\DIFdel{.
	}%DIFDELCMD < 

%DIFDELCMD < 	\bibitem{gautschiAuroraKinasesAnticancer2008}
%DIFDELCMD < 	%%%
\DIFdel{Gautschi O, Heighway J, Mack PC, Purnell PR, Lara PN, Gandara DR.
	}%DIFDELCMD < \newblock %%%
\DIFdel{Aurora }%DIFDELCMD < {{%%%
\DIFdel{Kinases}%DIFDELCMD < }} %%%
\DIFdel{as }%DIFDELCMD < {{%%%
\DIFdel{Anticancer Drug Targets}%DIFDELCMD < }}%%%
\DIFdel{.
	}%DIFDELCMD < \newblock %%%
\DIFdel{Clinical Cancer Research. 2008;14(6):1639--1648.
	}%DIFDELCMD < \newblock %%%
\DIFdel{doi:}%DIFDELCMD < {%%%
\DIFdel{10.1158/1078-0432.CCR-07-2179}%DIFDELCMD < }%%%
\DIFdel{.
	}%DIFDELCMD < 

%DIFDELCMD < 	\bibitem{riesCancerIncidenceSurvival1999}
%DIFDELCMD < 	%%%
...
\DIFdel{United States SEER Program}%DIFDELCMD < }} %%%
\DIFdel{1975-1995.
	}%DIFDELCMD < \newblock %%%
...
%DIFDELCMD < 	\bibitem{ringCheckpointProteinsPediatric2017}
%DIFDELCMD < 	%%%
\DIFdel{Ring EK, Markert JM, Gillespie GY, Friedman GK.
	}%DIFDELCMD < \newblock %%%
\DIFdel{Checkpoint }%DIFDELCMD < {{%%%
\DIFdel{Proteins}%DIFDELCMD < }} %%%
\DIFdel{in }%DIFDELCMD < {{%%%
\DIFdel{Pediatric Brain}%DIFDELCMD < }} %%%
\DIFdel{and }%DIFDELCMD < {{%%%
\DIFdel{Extracranial
			Solid Tumors}%DIFDELCMD < }}%%%
\DIFdel{: }%DIFDELCMD < {{%%%
\DIFdel{Opportunities}%DIFDELCMD < }} %%%
\DIFdel{for }%DIFDELCMD < {{%%%
\DIFdel{Immunotherapy}%DIFDELCMD < }}%%%
\DIFdel{.
	}%DIFDELCMD < \newblock %%%
\DIFdel{Clinical Cancer Research. 2017;23(2):342--350.
	}%DIFDELCMD < \newblock %%%
\DIFdel{doi:}%DIFDELCMD < {%%%
\DIFdel{10.1158/1078-0432.CCR-16-1829}%DIFDELCMD < }%%%
\DIFdel{.
	}%DIFDELCMD < 

%DIFDELCMD < 	\bibitem{hoadleyCellofOriginPatternsDominate2018}
%DIFDELCMD < 	%%%
\DIFdel{Hoadley KA, Yau C, Hinoue T, Wolf DM, Lazar AJ, Drill E, et~al.
	}%DIFDELCMD < \newblock %%%
\DIFdel{Cell-of-}%DIFDELCMD < {{%%%
\DIFdel{Origin Patterns Dominate}%DIFDELCMD < }} %%%
\DIFdel{the }%DIFDELCMD < {{%%%
\DIFdel{Molecular Classification}%DIFDELCMD < }}
%DIFDELCMD < 	%%%
\DIFdel{of 10,000 }%DIFDELCMD < {{%%%
\DIFdel{Tumors}%DIFDELCMD < }} %%%
\DIFdel{from 33 }%DIFDELCMD < {{%%%
\DIFdel{Types}%DIFDELCMD < }} %%%
\DIFdel{of }%DIFDELCMD < {{%%%
\DIFdel{Cancer}%DIFDELCMD < }}%%%
\DIFdel{.
	}%DIFDELCMD < \newblock %%%
\DIFdel{Cell. 2018;173(2):291--304.e6.
	}%DIFDELCMD < \newblock %%%
\DIFdel{doi:}%DIFDELCMD < {%%%
\DIFdel{10.1016/j.cell.2018.03.022}%DIFDELCMD < }%%%
\DIFdel{.
	}%DIFDELCMD < 

%DIFDELCMD < 	\bibitem{cunananEfficientBasketTrial2017}
%DIFDELCMD < 	%%%
\DIFdel{Cunanan KM, Iasonos A, Shen R, Begg CB, G}%DIFDELCMD < {%%%
\DIFdel{\"o}%DIFDELCMD < }%%%
\DIFdel{nen M.
	}%DIFDELCMD < \newblock %%%
\DIFdel{An }%DIFDELCMD < {{%%%
\DIFdel{Efficient Basket Trial Design}%DIFDELCMD < }}%%%
\DIFdel{.
	}%DIFDELCMD < \newblock %%%
\DIFdel{Statistics in medicine. 2017;36(10):1568--1579.
	}%DIFDELCMD < \newblock %%%
\DIFdel{doi:}%DIFDELCMD < {%%%
\DIFdel{10.1002/sim.7227}%DIFDELCMD < }%%%
\DIFdel{.
	}%DIFDELCMD < 

%DIFDELCMD < 	\bibitem{rheeImpactTumorPurity2018}
%DIFDELCMD < 	%%%
\DIFdel{Rhee JK, Jung YC, Kim KR, Yoo J, Kim J, Lee YJ, et~al.
	}%DIFDELCMD < \newblock %%%
\DIFdel{Impact of }%DIFDELCMD < {{%%%
\DIFdel{Tumor Purity}%DIFDELCMD < }} %%%
\DIFdel{on }%DIFDELCMD < {{%%%
\DIFdel{Immune Gene Expression}%DIFDELCMD < }} %%%
\DIFdel{and
	}%DIFDELCMD < {{%%%
\DIFdel{Clustering Analyses}%DIFDELCMD < }} %%%
\DIFdel{across }%DIFDELCMD < {{%%%
\DIFdel{Multiple Cancer Types}%DIFDELCMD < }}%%%
\DIFdel{.
	}%DIFDELCMD < \newblock %%%
\DIFdel{Cancer Immunology Research. 2018;6(1):87--97.
	}%DIFDELCMD < \newblock %%%
\DIFdel{doi:}%DIFDELCMD < {%%%
\DIFdel{10.1158/2326-6066.CIR-17-0201}%DIFDELCMD < }%%%
\DIFdel{.
	}%DIFDELCMD < 

%DIFDELCMD < 	\bibitem{raphael2017integrated}
%DIFDELCMD < 	%%%
