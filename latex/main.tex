...
In our experience, standard inference algorithms like the Gibbs sampler, tend to over-estimate the number of clusters such that samples belong to a separate cluster when they should be merged into a larger cluster that has more support. For this reason, we incorporated the bnpy memoized online variational inference algorithm (moVB) \cite{hughes2013memoized} into our analysis framework. The moVB algorithm uses variational inference algorithms and interleaves birth, merge, and delete moves, which was shown to generate reliable results by avoiding local optima and removing redundant clusters \cite{hughesBnpyReliableScalable}. 
...
The TARGET neuroblastoma and osteosarcoma RNA-Seq gene expression data were downloaded from the UCSC Xena Browser \cite{goldmanUCSCXenaPlatform2018}. The TARGET clinical data were downloaded from the TARGET Data Matrix (\url{https://ocg.cancer.gov/programs/target/data-matrix}). We trained the hydra pipeline on 70 International Neuroblastoma Staging System (INSS) stage 4, MYCN non-amplified neuroblastoma.  The resulting models were tested using an independent dataset of pediatric RNA-Seq data shared through the UCSC Treehouse compendium \url{https://treehousegenomics.soe.ucsc.edu/public-data/} \cite{vivianToilEnablesReproducible2017}. We also investigated 106 osteosarcoma, 82 Ewing sarcoma, 44 alveolar rhabdomyosarcoma, and 50 embryonal rhabdomyosarcoma gene expression profiles from the Treehouse compendium. We also included 58 synovial sarcoma microarray profiles (GEO: GSE40025) expression profiles in the pan-cancer analysis.
 
\subsection{TARGET MYCN-NA Neuroblastoma Analysis}
The neuroblastoma unsupervised enrichment analysis included all genes with a mean expression greater than 1.0 log2(TPM + 1), a minor expression component probability of at least 20\%, and a minimum effect size of 1.0. Our synthetic data analysis found that the detectable signal decreases below an effect size of 1.0, so we use this parameter for all analyses. We set a minimum mean threshold of 1.0 log2(TPM + 1) to exclude lowly expressed genes that may have unstable measurements. We used the hydra ScanEnrichmentAnalysis routine to search a range of minimum component probability thresholds and found that 20\% yielded the most number of clusters while keeping the number of genes (p=42) below the samples (n=70). ClusterProfiler identified statistically significant enrichment of GO Biological Process (GOBP) terms (FDR < 0.01) (Yu et al. 2012).

We then investigated correlations across significantly enrichment biological processes to identify clusters of samples that may define expression subtypes. The multivariate mixture model $\alpha$ concentration parameter was set to 5.0; the prior on the covariance matrix was set to the identity scaled by 2.0. The prior parameter for the number of clusters was 5 clusters. We correlated hydra expression clusters with the results of the tumor microenvironment profiling tools xCell \cite{aranXCellDigitallyPortraying2017} and ESTIMATE \cite{yoshiharaInferringTumourPurity2013}. We also applied the consensus clustering method M3C \cite{johnM3CMonteCarlo2018} and Gap statistic k-means method to the TARGET MYCN-NA neuroblastoma data across a range of gene thresolds using the largest median absolute deviation (MAD) \nameref{S3_Fig}. The number of clusters was inferred to be the smallest statistically significant value.


We used a smaller expression component probability for osteosarcoma to show the ability of the hydra approach to discover smaller but clear subtype expression signatures. The osteosarcoma unsupervised enrichment analysis included all genes with mean expression greater than 1.0 log2(TPM + 1), a minor expression component probability of at least 10\%, and a minimum effect size of 1.0.
...
\label{S3_Fig}
{\bf{Consensus clustering and k-means clustering do not detect all tumor microenvironment expression subtypes.}}
