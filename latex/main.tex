...
Traditional parametric models, like the finite mixture model, use a fixed number of parameters and suffer from over- and underfitting \cite{teh2010dirichlet}. Unlike the finite mixture model, the Dirichlet process mixture model adjusts the number of clusters given the data and prior belief. 

The Dirichlet process is related to the Dirichlet distribution, which is a distribution over probability vectors. The Dirichlet process extends the Dirichlet process to  and is used as a prior for cluster likelihoods. 
...
This features is useful for clinical applications of clustering because the number of clusters is usually not known beforehand and the number of
