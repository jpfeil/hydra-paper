...
%DIF ADD Pfeil-Hydra-Manuscript-V3.tex         Mon Jan  6 21:34:43 2020
...
\DIFdelbegin \DIFdel{The neuroblastoma unsupervised enrichment analysis }\DIFdelend \DIFaddbegin \DIFadd{We applied each hydra tool to the TARGET MYCN-NA neuroblastoma cohort. The input to the downstream clustering tools consists of the multimodal gene models. We first obtained these models }\DIFaddend using the hydra \textit{\DIFdelbegin \DIFdel{enrich}\DIFdelend \DIFaddbegin \DIFadd{filter}\DIFaddend } \DIFdelbegin \DIFdel{command included }\DIFdelend \DIFaddbegin \DIFadd{tool. The hydra }\textit{\DIFadd{filter}} \DIFadd{tool identified }\DIFaddend all genes with a \DIFdelbegin \DIFdel{mean expression greater than 1.0 }\DIFdelend \DIFaddbegin \DIFadd{multimodal expression pattern. We used the mean expression filter to remove genes that may have unstable measurements do to low transcript abundances. We excluded all genes with a mean expression value less than 1 }\DIFaddend log2(TPM + 1)\DIFdelbegin \DIFdel{, a minor expression }\DIFdelend \DIFaddbegin \DIFadd{. 
}
...
\DIFadd{The hydra }\textit{\DIFadd{enrich}} \DIFadd{command identified correlated expression signatures using the enriched GO term genes (FDR $<$ 0.01). The multivariate mixture model $\alpha$ concentration parameter was set to 5.0; the prior on the covariance matrix was set to the identity scaled by 2.0. The prior parameter for the number of clusters was set to five}\DIFaddend . Our synthetic data analysis found that the signal decreases below an effect size of 1.0, so we use this parameter value for all following analyses. We \DIFdelbegin \DIFdel{set a minimum mean threshold of 1.0 log2(TPM + 1) to exclude lowly expressed genes that may have unstable measurements. We }\DIFdelend used the hydra \textit{scan} routine to search a range of minimum component probability thresholds and found 20\% yielded the most clusters while keeping the number of genes (p = 42) below the number of samples (n = 70). 
...
\begin{figure}[!h]
	...
	\DIFaddbeginFL \includegraphics[width=\textwidth]{img/PNG/ROC-PLOT-V2-2x}
	\DIFaddendFL \caption{{\bf Hydra \DIFaddbeginFL \textit{\DIFaddFL{sweep}} \DIFaddendFL is more sensitive than existing gene set enrichment approaches for detecting differential pathway expression in synthetic data \DIFaddbeginFL \DIFaddFL{and scales well to large datasets}\DIFaddendFL .}
...
%DIFAUXCMD
\DIFdel{After completing the performance assessment with synthetic gene expression data, we applied the hydra unsupervised enrichment analysisto the TARGET high-risk neuroblastoma cohort.  
}\DIFdelend \DIFaddbegin \DIFadd{We performed a runtime analysis comparing hydra }\textit{\DIFadd{sweep}}\DIFadd{, ssGSEA, and GSVA for identifying a single differentially expressed gene-set, since these methods are directly comparable. The average runtime for the hydra }\textit{\DIFadd{sweep}} \DIFadd{algorithm was on average the same as ssGSEA and GSVA to run, but the hydra }\textit{\DIFadd{sweep}} \DIFadd{runtime was more variable because of the dependence on biological features of the expression cohort, whereas the GSEA-based approaches were independent of these features. The GSVA approach was faster than hydra }\textit{\DIFadd{sweep}} \DIFadd{and ssGSEA, but GSVA performed worse on synthetic data than ssGSEA and hydra. We repeated the above analysis, but with an effect size of 1.0, a \%DEG of 25\%, and a range of sample sizes, including  50, 100, 200, 300, 400, 500, 1000 samples. The hydra }\textit{\DIFadd{sweep}} \DIFadd{and GSVA methods scaled well to large sample sizes, but the ssGSEA runtime increased exponentially as the sample size increased (Fig \ref{rocplot}C \& D).  
}
...
%DIFDELCMD < %%%
\DIFdelend We applied the hydra \DIFdelbegin \DIFdel{unsupervised enrichment }\DIFdelend \DIFaddbegin \textit{\DIFadd{filter}} \DIFaddend analysis to the \DIFaddbegin \DIFadd{TARGET high-risk neuroblastoma cohort as described above. This analysis identified 931 genes within the }\DIFaddend MYCN-NA \DIFdelbegin \DIFdel{cohort. The multimodal expression filter identified }\DIFdelend \DIFaddbegin \DIFadd{neuroblastoma cohort with a multimodal expression distribution. Of the 931 multimodally expressed genes, 358 genes were found to be potentially druggable by the Drug Gene Interaction Database (DGiDB) and 61 genes were associated with an FDA-approved, anti-neoplastic drug  \mbox{%DIFAUXCMD
\cite{cotto2017dgidb}}\hspace{0pt}%DIFAUXCMD
. 
}

\DIFadd{We next looked to see if unsupervised clustering of multimodally expressed genes revealed coordinated expression of annotated gene sets within the MSigDB database. Applying the hydra }\textit{\DIFadd{sweep}} \DIFadd{command to the MYCN-NA neuroblastoma cohort discovered 105 gene sets with multimodal expression patterns. Each differentially expressed gene set sheds light on biological themes that are differentially expressed within the MYCN-NA neuroblastoma cohort. We clustered the differentially expressed gene-sets to reveal these biological themes (}\nameref{S1_Fig}\DIFadd{). We found 6 major themes, including annotated cancer functions, cell cycle regulation, cell signaling pathways, immune functions, extracellular matrix reorganization, and metabolic pathway gene sets.
}
...
\DIFaddbegin \begin{figure}[!h]
	...
	\caption{{\bf \DIFaddFL{Hydra analysis identifies three distinct tumor microenvironment expression subtypes in MYCN non-amplified neuroblastoma samples.}}
...
\DIFaddend \subsection*{N-of-1 \DIFdelbegin \DIFdel{Tumor Analysis }\DIFdelend \DIFaddbegin \DIFadd{tumor analysis }\DIFaddend for \DIFdelbegin \DIFdel{Pediatric Neuroblastoma}\DIFdelend \DIFaddbegin \DIFadd{pediatric neuroblastoma}\DIFaddend }
We investigated the predictive performance of the hydra approach for identifying clinically relevant signals in the N-of-1 tumor analysis setting. The command-line interface of the hydra toolkit includes a \textit{predict} function for labeling samples using a pre-fit model. The MYCN-NA neuroblastoma model described above was used to predict expression subtypes on a new set of samples. We obtained tumor gene expression data from \DIFdelbegin \DIFdel{five }\DIFdelend \DIFaddbegin \DIFadd{six }\DIFaddend stage 4, MYCN-NA neuroblastoma samples from the UCSC Treehouse gene expression compendium \cite{newtonTumorMapExploringMolecular2017, vaskeComparativeTumorRNA2019}. The age at diagnosis ranged from 2 to 6 years. Four out of \DIFdelbegin \DIFdel{five }\DIFdelend \DIFaddbegin \DIFadd{six }\DIFaddend samples had a deletion in the ATRX gene. Samples 1D and 1R were diagnosis and resection samples from the same patient. Most of the samples (4 out of \DIFdelbegin \DIFdel{5}\DIFdelend \DIFaddbegin \DIFadd{6}\DIFaddend ) clustered in cluster 1, which is characterized by adaptive immune pathway expression. \DIFaddbegin \DIFadd{H\&E image analysis confirmed the presence of moderate levels of inflammation in 3 cluster 1 samples, minimal inflammation in the cluster 2 sample, and moderate levels of inflammation and necrosis in the cluster 3 sample (}\nameref{S5_Fig}\DIFadd{).
}\DIFaddend 

Three of the ATRX-deleted samples clustered with the high immune expression cluster (cluster 1) and one clustered in the low immune, high proliferative signaling cluster (cluster 2). Hydra analysis assigned sample 1D to cluster 1 and sample 1R to cluster 2 despite both samples originating from the same patient. The resection sample 1R was extracted from lymph node tissue, which has a significantly different cellular composition than the training data. Another possible explanation for the different cluster assignments is that the tumor microenvironment is dynamic and the tumor may evade immune recognition as the disease progresses, resulting in different expression signatures. We performed GSEA comparing samples 1D and 1R to investigate potential mechanisms leading to immune evasion in sample 1R. GSEA found downregulation of the MHC Class I Antigen Processing \& Presentation GO term in sample 1R (adjusted p-value $<$ 0.002). Loss of antigen processing functions is a common mechanism of immune evasion across cancer types \cite{reevesAntigenProcessingImmune2017}. \DIFdelbegin %DIFDELCMD < 
...
\DIFdel{). For example, enrichment of immune expression signatures within cluster 2 predicted differences in overall survival such that patients with }\DIFdelend \DIFaddbegin \DIFadd{We showed earlier that tumors with ATRX deletions tend to have }\DIFaddend higher immune expression\DIFdelbegin \DIFdel{had a better overall survival rate. Similarly, an elevated cell cycle signal within cluster 3 predicted worse survival compared to other cluster 3 samples with lower cell cycle expression. A metastatic expression signal was identified in the analysis of cluster 1 samples, but this signature did not correlate with a difference in survival. This approach may therefore provide a more appropriate background distribution for revealing and evaluating the significance of gene expression patterns and survival statistics within tumor subtypes}\DIFdelend \DIFaddbegin \DIFadd{, so our N-of-1 analysis on a new set of MYCN-NA neuroblastoma is consistent with our earlier analysis}\DIFaddend . 
...
Multivariate gene expression analysis is typically underpowered because the number of genes greatly exceeds the number of samples. To address this limitation, we propose selecting for multimodally expressed genes before performing multivariate analysis. The hydra \DIFdelbegin \DIFdel{multimodal filter }\DIFdelend \DIFaddbegin \textit{\DIFadd{filter}} \DIFaddend reduces the number of genes and enriches for genes that participate in known biological processes, including those curated in the Gene Ontology \DIFdelbegin \DIFdel{database. For example, }\DIFdelend \DIFaddbegin \DIFadd{and MSigDB databases. Furthermore, we found that }\DIFaddend multimodally expressed genes \DIFdelbegin \DIFdel{separate neuroblastoma subtypes }\DIFdelend \DIFaddbegin \DIFadd{improves separation of known clinical subtypes better }\DIFaddend by TumorMap analysis \DIFdelbegin \DIFdel{better than }\DIFdelend \DIFaddbegin \DIFadd{compared to }\DIFaddend the standard approach of using all expressed genes \DIFaddbegin \DIFadd{(}\nameref{S2_Fig}\DIFadd{)}\DIFaddend . Furthermore, we showed that the hydra approach is more sensitive at \DIFaddbegin \DIFadd{identifying differential pathway expression and }\DIFaddend resolving tumor microenvironment subtypes than \DIFdelbegin \DIFdel{the M3C consensus clustering and k-means gap-statistic approach}\DIFdelend \DIFaddbegin \DIFadd{single-sample GSEA methods and consensus clustering approaches, respectively}\DIFaddend . 

Significant progress has been made in subtyping neuroblastomas and adapting therapy for aggressive subtypes, but unexplained heterogeneity remains \cite{morgensternChallengeDefiningUltrahighrisk2019}. Failure to account for this heterogeneity decreases the power of standard methods to detect important expression patterns. Identifying biomarkers using genome-wide technology may lead to improved risk stratification and the discovery of novel drug targets. Hydra analysis of the TARGET MYCN-NA neuroblastoma cohort \DIFdelbegin \DIFdel{(N=70) }\DIFdelend found differential expression of tumor microenvironment markers, including markers of the adaptive immune response. Pediatric cancers are generally thought to be less immunogenic because they have lower mutation burdens than adult cancers, but the immunogenicity of pediatric cancer has not been sufficiently investigated \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
\DIFadd{Unsupervised clustering of multimodal gene sets revealed biological themes associated with cancer functions. cell cycle, immune functions, extracellular matrix organization, and metabolism}\DIFaddend .

\paragraph*{S5 Fig.}
%DIF < \includegraphics[width=\textwidth]{img/muscle-signature-genes-2x}
\DIFaddbegin \includegraphics[width=\textwidth]{img/PNG/HE-Figure-3-Clusters-2x}
\DIFaddend \label{S5_Fig} {\bf Hydra \DIFdelbegin \DIFdel{analysis of TARGET osteosarcoma cohort reveals skeletal muscle signature}\DIFdelend \DIFaddbegin \DIFadd{method correlates with distinct tumor features as assessed by licensed pathologist review of tumor H\&E slides}\DIFaddend .}
\DIFdelbegin \DIFdel{Hydra enrichment analysis on the TARGET osteosarcoma cohort revealed a subset of patients with high skeletal muscle expression. A: Clustered heatmap shows the muscle signature genes identified by hydra unsupervised enrichment analysis. B: xCell tumor microenvironment profiling identified significant differences in skeletal muscle expression compared to background (p }\DIFdelend \DIFaddbegin \DIFadd{A-B: H\&E sections from fresh frozen tumor tissue from MYCN-NA neuroblastoma sample at A: 2X magnification and B: 20X magnification. Tumor cells are medium to large with moderate amounts of cytoplasm and areas of rhabdoid appearing undifferentiated cells.  There is a moderate amount of mixed inflammation present (30-50\%) consisting mostly of mature mononuclear cells with some plasma cells and scattered eosinophils. C-D: H\&E sections from fresh frozen tumor tissue from MYCN-NA neuroblastoma at C: 2X magnification and D: 20X magnification. Tumor cells are moderate to large in size with moderate amounts of cytoplasm. There is a minimal amount (}\DIFaddend $<$\DIFdelbegin \DIFdel{0.001) . C}\DIFdelend \DIFaddbegin \DIFadd{10\%) of apparent mononuclear inflammation scattered throughout the tumor. E-F}\DIFaddend : H\&E \DIFdelbegin \DIFdel{stained tumor slide for an independent osteosarcoma sample confirms presence of striated muscle tissue within the sequenced tumor sample }\DIFdelend \DIFaddbegin \DIFadd{sections from fresh frozen tumor tissue from MYCN-NA neuroblastoma sample at (E) 2X magnification and (F) 20X magnification.  Tumor cells are medium to large with moderate amounts of cytoplasm and areas of rhabdoid appearing undifferentiated cells.  There are also areas of apparent necrosis.  There is a moderate amount of inflammation present (30-50\%) consisting mostly of mature mononuclear cells with some plasma cells and scattered eosinophils}\DIFaddend .

% Add panel labels
\paragraph*{S6 Fig.}
%DIF < \includegraphics[width=\textwidth]{img/Selection_026}
\DIFaddbegin \includegraphics[width=\textwidth]{img/PNG/Checkpoint-Plots-V1}
\DIFaddend \label{S6_Fig} {\bf Hydra \DIFaddbegin \textit{\DIFadd{enrich}} \DIFadd{analysis identifies correlation between expression subtypes and checkpoint blockade markers in MYCN-NA neuroblastoma.}} 

%DIF >  Add panel labels
\paragraph*{\DIFadd{S7 Fig.}}
\includegraphics[width=\textwidth]{img/PNG/ewing-jak1-expression-fig}
\label{S7_Fig} {\bf \DIFadd{Hydra }\DIFaddend analysis identifies correlation between expression of druggable JAK1 gene and mast cell expression signature in Ewing sarcoma.} A: We show the utility of the mixture model approach for identifying important gene expression patterns for precision medicine applications. JAK1 is a druggable gene, but JAK1 is also an important gene in immune cell signaling pathways. B: Boxplot showing the xCell mast cell enrichment score for three clusters associated with JAK1 expression.
...
\DIFaddend {\bf \DIFdelbegin \DIFdel{Hydra analysis identifies tumor microenvironment expression subtypes that correlate with patient outcomes in osteosarcoma and synovial sarcoma}\DIFdelend \DIFaddbegin \DIFadd{TARGET MYCN-NA neuroblastoma supplementary data}\DIFaddend .}
...
\DIFdel{JAMA Network Open. 2019}\DIFdelend ;2(10):e1913968--e1913968.
	...
	Microenvironment\DIFdelbegin \DIFdel{.
	}%DIFDELCMD < \newblock %%%
\DIFdel{Science (New York, NY). 2015}\DIFdelend ;348(6230):74--80.
	\newblock doi:{10.1126/science.aaa6204}.

	\bibitem{chenElementsCancerImmunity2017}
	Chen DS, Mellman I.
	\newblock Elements of Cancer Immunity and the Cancer\DIFdelbegin \DIFdel{\textendash{}}\DIFdelend \DIFaddbegin \DIFadd{–}\DIFaddend Immune Set
	Point\DIFdelbegin \DIFdel{.
	}%DIFDELCMD < \newblock %%%
\DIFdel{Nature. 2017}\DIFdelend ;541(7637):321--330.
	\newblock doi:{10.1038/nature21349}.

	\bibitem{mellmanCancerImmunotherapyComes2011}
	Mellman I, Coukos G, Dranoff G.
	\newblock Cancer Immunotherapy Comes of Age\DIFdelbegin \DIFdel{.
	}%DIFDELCMD < \newblock %%%
\DIFdel{Nature. 2011}\DIFdelend ;480(7378):480--489.
	\newblock doi:{10.1038/nature10673}.

	\bibitem{pageImmuneModulationCancer2014}
	Page DB, Postow MA, Callahan MK, Allison JP, Wolchok JD.
	\newblock Immune Modulation in Cancer with Antibodies\DIFdelbegin \DIFdel{.
	}%DIFDELCMD < \newblock %%%
\DIFdel{Annual Review of Medicine. 2014}\DIFdelend ;65:185--202.
	...
	Abundant and Functional Neoantigen-Specific }{{\DIFadd{CD8}}}\DIFadd{+ }{{\DIFadd{T}}} \DIFadd{Cell
	Responses;11(498).
	...
	Data Using {{R}} and {{Bioconductor}}\DIFdelbegin \DIFdel{.
	}%DIFDELCMD < \newblock %%%
\DIFdel{Nature Protocols. 2013}\DIFdelend ;8(9):1765--1786.
	\newblock doi:{10.1038/nprot.2013.099}.

	\bibitem{andersDifferentialExpressionAnalysis2010}
	Anders S, Huber W.
	\newblock Differential Expression Analysis for Sequence Count Data\DIFdelbegin \DIFdel{.
	}%DIFDELCMD < \newblock %%%
\DIFdel{Genome Biology. 2010}\DIFdelend ;11(10):R106.
...
\DIFdel{Nature Genetics. 2003}\DIFdelend ;34(3):267--273.
	...
	Liberzon A, Subramanian A, Pinchback R, \DIFdelbegin \DIFdel{Thorvaldsd}%DIFDELCMD < {%%%
\DIFdel{\'o}%DIFDELCMD < }%%%
...
\DIFdel{Bioinformatics (Oxford, England). 2010}\DIFdelend ;26(12):1572--1573.
	...
	Lenz M, \DIFdelbegin \DIFdel{M}%DIFDELCMD < {%%%
\DIFdel{\"u}%DIFDELCMD < }%%%
\DIFdel{ller }\DIFdelend \DIFaddbegin \DIFadd{Müller }\DIFaddend FJ, Zenke M, Schuppert A.
	\newblock Principal Components Analysis and the Reported Low Intrinsic
	Dimensionality of Gene Expression Microarray Data\DIFdelbegin \DIFdel{.
	}%DIFDELCMD < \newblock %%%
\DIFdel{Scientific Reports. 2016}\DIFdelend ;6(1):1--11.
	...
	{{Bioengineering}} ({{BIBE}}'05);\DIFdelbegin \DIFdel{2005. }\DIFdelend \DIFaddbegin \DIFadd{. }\DIFaddend p. 276--280.

	\bibitem{ghoshMixtureModelsAssessing2004}
	Ghosh D.
	\newblock Mixture Models for Assessing Differential Expression in Complex
	Tissues Using Microarray Data\DIFdelbegin \DIFdel{.
	}%DIFDELCMD < \newblock %%%
\DIFdel{Bioinformatics (Oxford, England). 2004}\DIFdelend ;20(11):1663--1669.
	\newblock doi:{10.1093/bioinformatics/bth139}.

	\bibitem{dahlModelBasedClusteringExpression2006}
	Dahl DB, Vannucci M.
	\newblock \DIFdelbegin \DIFdel{Model-}%DIFDELCMD < {{%%%
\DIFdel{Based Clustering}%DIFDELCMD < }} %%%
\DIFdel{for }%DIFDELCMD < {{%%%
\DIFdel{Expression Data}%DIFDELCMD < }} %%%
\DIFdel{via a }%DIFDELCMD < {{%%%
\DIFdel{Dirichlet
			Process Mixture Model}%DIFDELCMD < }}%%%
\DIFdel{.
	}%DIFDELCMD < \newblock %%%
\DIFdelend In: Do KA, Muller P, editors. \DIFdelbegin \DIFdel{Bayesian }\DIFdelend \DIFaddbegin \DIFadd{Model-}\DIFaddend {{\DIFdelbegin \DIFdel{Inference}\DIFdelend \DIFaddbegin \DIFadd{Based Clustering}\DIFaddend }} for
	{{\DIFdelbegin \DIFdel{Gene
			Expression }\DIFdelend \DIFaddbegin \DIFadd{Expression Data}\DIFaddend }} \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{via a }\DIFaddend {{\DIFdelbegin \DIFdel{Proteomics}\DIFdelend \DIFaddbegin \DIFadd{Dirichlet Process Mixture Model}\DIFaddend }}. {Cambridge
		\DIFdelbegin %DIFDELCMD < }%%%
\DIFdel{: }%DIFDELCMD < {%%%
\DIFdel{Cambridge }\DIFdelend University Press};\DIFdelbegin \DIFdel{2006. }\DIFdelend \DIFaddbegin \DIFadd{. }\DIFaddend p. 201--218.
	\DIFaddbegin \newblock \DIFadd{Available from:
	}\url{https://www.cambridge.org/core/product/identifier/CBO9780511584589A070/type/book_part}\DIFadd{.
	}\DIFaddend 
...
\DIFdel{Biometrika. 2006}\DIFdelend ;93(4):877--893.
	...
	\newblock {Chapman and Hall/CRC};\DIFdelbegin \DIFdel{2013.
...
\DIFdel{The Annals of Statistics. 1973}\DIFdelend ;1(2):209--230.
	...
	\DIFdelbegin \DIFdel{M}%DIFDELCMD < {%%%
\DIFdel{\"u}%DIFDELCMD < }%%%
...
\DIFdel{Statistical science. 2004}\DIFdelend ; p. 95--110.
	...
	\DIFdelbegin \DIFdel{G}%DIFDELCMD < {%%%
\DIFdel{\"o}%DIFDELCMD < }%%%
\DIFdel{r}%DIFDELCMD < {%%%
\DIFdel{\"u}%DIFDELCMD < }%%%
...
\DIFdel{Journal of Computer Science and Technology. 2010}\DIFdelend ;25(4):653--664.
	...
	Mixture Models.
	\newblock In: Advances in Neural Information Processing Systems;\DIFdelbegin \DIFdel{2013. }\DIFdelend \DIFaddbegin \DIFadd{. }\DIFaddend p.
	1133--1141.

	\DIFaddbegin \bibitem{mullerBayesianNonparametricData2015}
	\DIFadd{Müller P, Quintana FA, Jara A, Hanson T.
		...
		Publishing}}\DIFadd{;.
	}\newblock \DIFadd{Available from: }\url{https://www.springer.com/gp/book/9783319189673}\DIFadd{.
	}

	\bibitem{phadia2015prior}
	\DIFadd{Phadia EG.
	}\newblock \DIFadd{Prior Processes and Their Applications.
	}\newblock {\DIFadd{Springer}}\DIFadd{;.
	}
	...
	\newblock Gene {{Ontology}}: Tool for the Unification of Biology\DIFdelbegin \DIFdel{.
	}%DIFDELCMD < \newblock %%%
\DIFdel{Nature Genetics. 2000}\DIFdelend ;25(1):25--29.
	\newblock doi:{10.1038/75556}.

	\bibitem{gene2018gene}
	Consortium GO.
	\newblock The Gene Ontology Resource: 20 Years and Still {{GOing}}
	Strong\DIFdelbegin \DIFdel{.
	}%DIFDELCMD < \newblock %%%
\DIFdel{Nucleic acids research. 2018}\DIFdelend ;47(D1):D330--D338.
	...
	Visualization and Interpretation\DIFdelbegin \DIFdel{.
	}%DIFDELCMD < \newblock %%%
\DIFdel{PloS one. 2010}\DIFdelend ;5(11):e13984.
	...
	among Gene Clusters\DIFdelbegin \DIFdel{.
	}%DIFDELCMD < \newblock %%%
\DIFdel{Omics: A Journal of Integrative Biology. 2012}\DIFdelend ;16(5):284--287.
	...
	\newblock The {{Limitations}} of {{Simple Gene Set Enrichment Analysis Assuming
			Gene Independence}}\DIFdelbegin \DIFdel{.
	}%DIFDELCMD < \newblock %%%
\DIFdel{Statistical methods in medical research. 2016}\DIFdelend ;25(1):472--487.
	\newblock doi:{10.1177/0962280212460441}.

	\bibitem{korotkevichFastGeneSet2019}
	Korotkevich G, Sukhov V, Sergushichev A.
	\newblock Fast Gene Set Enrichment Analysis\DIFdelbegin \DIFdel{.
	}%DIFDELCMD < \newblock %%%
\DIFdel{bioRxiv. 2019}\DIFdelend ; p. 060012.
	...
	{{KRAS}}-Driven Cancers Require {{TBK1}}\DIFdelbegin \DIFdel{.
	}%DIFDELCMD < \newblock %%%
\DIFdel{Nature. 2009}\DIFdelend ;462(7269):108--112.
	\newblock doi:{10.1038/nature08460}.

	\bibitem{hanzelmannGSVAGeneSet2013}
	\DIFdelbegin \DIFdel{H}%DIFDELCMD < {%%%
\DIFdel{\"a}%DIFDELCMD < }%%%
\DIFdel{nzelmann }\DIFdelend \DIFaddbegin \DIFadd{Hänzelmann }\DIFaddend S, Castelo R, Guinney J.
	\newblock {{GSVA}}: Gene Set Variation Analysis for Microarray and
	{{RNA}}-{{Seq}} Data\DIFdelbegin \DIFdel{.
	}%DIFDELCMD < \newblock %%%
\DIFdel{BMC Bioinformatics. 2013}\DIFdelend ;14(1):7.
	...
	{{Sensitivity}}, {{Prioritization}} and {{Specificity}}\DIFdelbegin \DIFdel{.
	}%DIFDELCMD < \newblock %%%
\DIFdel{PLOS ONE. 2013}\DIFdelend ;8(11):e79217.
	...
	of {{Prognostic Gene Signatures}}\DIFdelbegin \DIFdel{.
	}%DIFDELCMD < \newblock %%%
\DIFdel{PLOS ONE. 2014}\DIFdelend ;9(1):e85150.
	\newblock doi:{10.1371/journal.pone.0085150}.

	\bibitem{lagardeChromosomeInstabilityAccounts2013}
	Lagarde P, Przybyl J, Brulard C, \DIFdelbegin \DIFdel{P}%DIFDELCMD < {%%%
\DIFdel{\'e}%DIFDELCMD < }%%%
\DIFdel{rot }\DIFdelend \DIFaddbegin \DIFadd{Pérot }\DIFaddend G, Pierron G, Delattre O, et~al.
	\newblock Chromosome Instability Accounts for Reverse Metastatic Outcomes of
	Pediatric and Adult Synovial Sarcomas\DIFdelbegin \DIFdel{.
	}%DIFDELCMD < \newblock %%%
\DIFdel{Journal of Clinical Oncology: Official Journal of the American
	Society of Clinical Oncology. 2013}\DIFdelend ;31(5):608--615.
...
\DIFdel{Nature Methods. 2015}\DIFdelend ;12(5):453--457.
	...
	Yoshihara K, Shahmoradgoli M, \DIFdelbegin \DIFdel{Mart}%DIFDELCMD < {%%%
\DIFdel{\'i}%DIFDELCMD < }%%%
\DIFdel{nez }\DIFdelend \DIFaddbegin \DIFadd{Martínez }\DIFaddend E, Vegesna R, Kim H, \DIFdelbegin %DIFDELCMD < {%%%
\DIFdelend Torres-Garcia \DIFdelbegin %DIFDELCMD < }
%DIFDELCMD < 	%%%
\DIFdelend W,
	et~al.
...
\DIFdel{Journal of machine learning research. 2011}\DIFdelend ;12\DIFdelbegin \DIFdel{(Oct)}\DIFdelend :2825--2830.
	...
	\newblock {{SciPy}}: {{Open}} Source Scientific Tools for {{Python}}\DIFdelbegin \DIFdel{. 2001}\DIFdelend ;.

	\bibitem{2019arXiv190710121V}
	Virtanen P, Gommers R, Oliphant TE, Haberland M, Reddy T, Cournapeau D, et~al.
	\newblock {{SciPy}} 1.0\DIFdelbegin \DIFdel{\textendash{{Fundamental Algorithms}} }\DIFdelend \DIFaddbegin \DIFadd{–}{{\DIFadd{Fundamental Algorithms}}} \DIFaddend for {{Scientific
...
\DIFdel{arXiv e-prints. 2019}\DIFdelend ; p. arXiv:1907.10121.
	...
	{{Python}}\DIFdelbegin \DIFdel{.
	}%DIFDELCMD < \newblock %%%
\DIFdel{The Journal of Open Source Software. 2019}\DIFdelend ;4(36):1169.
	\newblock doi:{10.21105/joss.01169}.

	\bibitem{kassambaraSurvminerDrawingSurvival2019}
	Kassambara A, Kosinski M, Biecek P.
	\newblock Survminer: Drawing Survival Curves Using 'Ggplot2';\DIFdelbegin \DIFdel{2019.
	}\DIFdelend \DIFaddbegin \DIFadd{.
	}\newblock \DIFadd{Available from: }\url{https://CRAN.R-project.org/package=survminer}\DIFadd{.
	}\DIFaddend 
	...
	\newblock The Challenge of Defining \DIFdelbegin \DIFdel{``}\DIFdelend \DIFaddbegin \DIFadd{“}\DIFaddend Ultra-High-Risk\DIFdelbegin \DIFdel{'' Neuroblastoma.
	}%DIFDELCMD < \newblock %%%
\DIFdel{Pediatric Blood \& Cancer. 2019}\DIFdelend \DIFaddbegin \DIFadd{”
	Neuroblastoma}\DIFaddend ;66(4):e27556.
	...
	}\newblock {{\DIFadd{DGIdb}}} \DIFadd{3.0: A Redesign and Expansion of the Drug–Gene
	...
	\newblock The Evolving Relationship of Wound Healing and Tumor Stroma\DIFdelbegin \DIFdel{.
	}%DIFDELCMD < \newblock %%%
\DIFdel{JCI Insight}\DIFdelend ;3(18).
	...
	Experiments\DIFdelbegin \DIFdel{.
	}%DIFDELCMD < \newblock %%%
\DIFdel{Proceedings of the National Academy of Sciences.
	2010}\DIFdelend ;107(21):9546--9551.
	...
	\newblock Filtering {{Genes}} for {{Cluster}} and {{Network
			Analysis}}\DIFdelbegin \DIFdel{.
	}%DIFDELCMD < \newblock %%%
\DIFdel{BMC Bioinformatics. 2009}\DIFdelend ;10(1):193.
	\newblock doi:{10.1186/1471-2105-10-193}.

	\bibitem{carcamo-oriveAnalysisTranscriptionalVariability2017}
	\DIFdelbegin %DIFDELCMD < {%%%
\DIFdelend Carcamo-Orive \DIFdelbegin %DIFDELCMD < } %%%
\DIFdelend I, Hoffman GE, Cundiff P, Beckmann ND, D\DIFdelbegin \DIFdel{'}\DIFdelend \DIFaddbegin \DIFadd{’}\DIFaddend Souza SL, Knowles JW,
	et~al.
	...
	{{Heterogeneity}}\DIFdelbegin \DIFdel{.
	}%DIFDELCMD < \newblock %%%
\DIFdel{Cell Stem Cell. 2017}\DIFdelend ;20(4):518--532.e9.
	\newblock doi:{10.1016/j.stem.2016.11.005}.

	\bibitem{maechler2012cluster}
	Maechler M, Rousseeuw P, Struyf A, Hubert M, Hornik K, et~al.
	\newblock Cluster: Cluster Analysis Basics and Extensions\DIFdelbegin \DIFdel{.
	}%DIFDELCMD < \newblock %%%
\DIFdel{R package version. 2012}\DIFdelend ;1(2):56.

	\bibitem{reevesAntigenProcessingImmune2017}
	Reeves E, James E.
	\newblock Antigen Processing and Immune Regulation in the Response to
	Tumours\DIFdelbegin \DIFdel{.
	}%DIFDELCMD < \newblock %%%
\DIFdel{Immunology. 2017}\DIFdelend ;150(1):16--24.
	\newblock doi:{10.1111/imm.12675}.

	\bibitem{hermansJAK1JAK2Inhibitor2018}
	Hermans MAW, Schrijver B, \DIFdelbegin %DIFDELCMD < {%%%
\DIFdelend van Holten-Neelen \DIFdelbegin %DIFDELCMD < } %%%
\DIFdel{CCPA, }%DIFDELCMD < {%%%
\DIFdel{Gerth}\DIFdelend \DIFaddbegin \DIFadd{CCPA, Gerth~van Wijk R, }\DIFaddend van \DIFdelbegin \DIFdel{Wijk}%DIFDELCMD < } %%%
\DIFdel{R, }%DIFDELCMD < {%%%
\DIFdel{van Hagen
	}%DIFDELCMD < } %%%
\DIFdel{PM, }%DIFDELCMD < {%%%
\DIFdel{van Daele }%DIFDELCMD < } %%%
\DIFdelend \DIFaddbegin \DIFadd{Hagen
	PM, van Daele }\DIFaddend PLA, et~al.
	\newblock The {{JAK1}}/{{JAK2}}- Inhibitor Ruxolitinib Inhibits Mast Cell
	Degranulation and Cytokine Release\DIFdelbegin \DIFdel{.
	}%DIFDELCMD < \newblock %%%
\DIFdel{Clinical and Experimental Allergy: Journal of the British Society for
	Allergy and Clinical Immunology. 2018}\DIFdelend ;48(11):1412--1420.
	...
	}{{\DIFadd{Pediatric Preclinical Testing Program}}} \DIFadd{(}{{\DIFadd{PPTP}}}\DIFadd{);55(1):26--34.
	...
	}\newblock \DIFadd{Aurora }{{\DIFadd{Kinases}}} \DIFadd{as }{{\DIFadd{Anticancer Drug Targets}}}\DIFadd{;14(6):1639--1648.
	...
	of 10,000 {{Tumors}} from 33 {{Types}} of {{Cancer}}\DIFdelbegin \DIFdel{.
	}%DIFDELCMD < \newblock %%%
\DIFdel{Cell. 2018}\DIFdelend ;173(2):291--304.e6.
	\newblock doi:{10.1016/j.cell.2018.03.022}.

	\DIFdelbegin %DIFDELCMD < \bibitem{majzner2017harnessing}
%DIFDELCMD < 	%%%
...
\DIFdel{Cancer Cell. 2017;31(4):476--485.
	}%DIFDELCMD < 

%DIFDELCMD < 	%%%
\DIFdelend \bibitem{cunananEfficientBasketTrial2017}
	Cunanan KM, Iasonos A, Shen R, Begg CB, \DIFdelbegin \DIFdel{G}%DIFDELCMD < {%%%
\DIFdel{\"o}%DIFDELCMD < }%%%
\DIFdel{nen }\DIFdelend \DIFaddbegin \DIFadd{Gönen }\DIFaddend M.
	\newblock An {{Efficient Basket Trial Design}}\DIFdelbegin \DIFdel{.
	}%DIFDELCMD < \newblock %%%
\DIFdel{Statistics in medicine. 2017}\DIFdelend ;36(10):1568--1579.
	...
	{{Clustering Analyses}} across {{Multiple Cancer Types}}\DIFdelbegin \DIFdel{.
	}%DIFDELCMD < \newblock %%%
\DIFdel{Cancer Immunology Research. 2018}\DIFdelend ;6(1):87--97.
	...
	Adenocarcinoma\DIFdelbegin \DIFdel{.
	}%DIFDELCMD < \newblock %%%
\DIFdel{Cancer cell. 2017}\DIFdelend ;32(2):185--203.
