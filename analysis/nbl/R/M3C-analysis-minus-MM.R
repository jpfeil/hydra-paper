...
pth <- 'data/NO-MM-target-high-risk-nbl-mycn-na-exp-2018-11-12.tsv'
d <- read.table(pth, 
                sep='\t', 
                header=T, 
                row.names = 1)
...
pth1 <- 'data/cluster-analysis/NO-MM-MYCN-NA-M3C.tsv'
...
m3c <- function(thresh, k){
  ...
  write.table(res$scores, 
              file=paste('data/cluster-analysis/NO-MM-MYCN-NA-M3C-MAD-', thresh, '.tsv', sep = ''),
              sep='\t')
  write.table(res$realdataresults[[k]]$ordered_annotation,
              file=paste('data/cluster-analysis/NO-MM-MYCN-NA-M3C-MAD-', thresh, '-labels-', k, '.tsv', sep = ''),
              sep='\t')
  return(res)
}
...
gapstat <- function(thresh, k){
  ...
  write.table(kmeans$cluster,
              file=paste('data/cluster-analysis/NO-MM-MYCN-NA-GAP-MAD-', thresh, '-labels-', k, '.tsv', sep = ''),
              sep='\t')
...
### 500 MAD Genes ###
res500 <- m3c(500, 2) # Confirmed Lost third cluster
res500$scores
...
### 100 MAD Genes ###
res100 <- m3c(100, 8) # Confirmed 
res100$scores

# GAP statistic 
kgap100 <- gapstat(100, 3) # Confirmed
